# %%writefile /kaggle/working/rsna_w6_curia_teacher_spatial.py
#!/usr/bin/env python3
# ============================================================
...
CURIA_REVISION = os.environ.get(
    "W6_CURIA_REVISION", "0ab30ca2b47f459b9fdc085d37d2784e6c0d7ec9"
).strip()
...
# Online repair/download target. Keep this on the native Linux filesystem rather
# than the project's /media NTFS mount. A shallow Git clone of a Hugging Face
# model commonly leaves model.safetensors as a ~130-byte Git-LFS pointer.
CURIA_DOWNLOAD_ROOT = Path(
    os.environ.get(
        "W6_CURIA_DOWNLOAD_ROOT",
        str(Path.home() / ".cache" / "rsna_w6" / "curia-2"),
    )
).expanduser()
CURIA_AUTO_DOWNLOAD_SETTING = (
    os.environ.get("W6_CURIA_AUTO_DOWNLOAD", "auto").strip().lower()
)
CURIA_REQUIRED_FILES = [
    "config.json",
    "model.safetensors",
    "preprocessor_config.json",
    "curia_image_processor.py",
]
CURIA_MIN_REAL_WEIGHT_BYTES = 300_000_000
...
TRAIN_GPU_IDS_TEXT = os.environ.get("W6_TRAIN_GPU_IDS", CACHE_GPU_IDS_TEXT).strip()
INFER_GPU_IDS_TEXT = os.environ.get("W6_INFER_GPU_IDS", CACHE_GPU_IDS_TEXT).strip()
PARALLEL_TRAINING = os.environ.get("W6_PARALLEL_TRAINING", "1").strip().lower() not in {
    "0",
    "false",
    "no",
    "off",
}
PARALLEL_INFERENCE = os.environ.get(
    "W6_PARALLEL_INFERENCE", "1"
...
def _curia_auto_download_allowed() -> bool:
    setting = CURIA_AUTO_DOWNLOAD_SETTING
    if setting in {"1", "true", "yes", "on"}:
        return True
    if setting in {"0", "false", "no", "off"}:
        return False
    # "auto": allow on normal/local machines, but never depend on internet in
    # Kaggle's hidden offline rerun. Kaggle must attach real Curia assets.
    return not IS_KAGGLE


def parse_pointer_file(path: Path) -> Optional[Dict[str, Any]]:
    """Return Git-LFS/Xet pointer metadata when *path* is a tiny pointer file."""
    try:
    ...
    if not first.startswith("version https://"):
        return None
    out: Dict[str, Any] = {"is_pointer": True, "first_line": first}
    for line in text.splitlines():
        ...
        if line.startswith("oid sha256:"):
            ...
            try:
                out["remote_size_bytes"] = int(line.split(None, 1)[1])
            except Exception:
                out["remote_size_raw"] = line
    return out


def curia_root_health(root: Path) -> Dict[str, Any]:
    root = Path(root)
    ...
    result: Dict[str, Any] = {
        ...
        "exists": root.exists(),
        "required_files": {
            name: (root / name).exists() for name in CURIA_REQUIRED_FILES
        },
    }
    if not weights.exists():
        result.update({"ok": False, "reason": "model.safetensors missing"})
        return result
    result["file_size_bytes"] = int(weights.stat().st_size)
    pointer = parse_pointer_file(weights)
    if pointer is not None:
        result["pointer"] = pointer
        result.update(
            {
                "ok": False,
                "reason": "model.safetensors is a Git-LFS/Xet pointer, not model weights",
            }
        )
        return result
    header = inspect_safetensors_header(weights)
    result["header_valid"] = bool(header.get("header_valid", False))
    if weights.stat().st_size < CURIA_MIN_REAL_WEIGHT_BYTES:
        result.update(
            {
                "ok": False,
                "reason": f"weight file too small ({weights.stat().st_size} bytes)",
            }
        )
        return result
    if not header.get("header_valid", False):
        result.update(
            {
                "ok": False,
                "reason": f"invalid safetensors header: {header.get('header_error')}",
            }
        )
        return result
    try:
        ...
        result["sha256"] = sha
        if sha != EXPECTED_CURIA_MODEL_SHA256 and not ALLOW_CURIA_SHA_MISMATCH:
            result.update({"ok": False, "reason": f"SHA mismatch: {sha}"})
            return result
    except Exception as exc:
        result.update({"ok": False, "reason": f"SHA calculation failed: {exc!r}"})
        return result
    result["ok"] = True
    result["reason"] = "verified Curia-2 weights"
    return result


def download_curia_snapshot(force: bool = False) -> Path:
    """Download the pinned public Curia-2 snapshot and verify its weight bytes/SHA."""
    try:
        from huggingface_hub import snapshot_download
    except Exception as exc:
        raise RuntimeError(
            "huggingface_hub is required for Curia auto-download. Install it with "
            "`pip install -U huggingface_hub`."
        ) from exc

    CURIA_DOWNLOAD_ROOT.mkdir(parents=True, exist_ok=True)
    log(
        f"[Curia] downloading {CURIA_REPO_ID}@{CURIA_REVISION[:12]} to "
        f"{CURIA_DOWNLOAD_ROOT}"
    )
    snapshot = snapshot_download(
        repo_id=CURIA_REPO_ID,
        revision=CURIA_REVISION,
        local_dir=str(CURIA_DOWNLOAD_ROOT),
        allow_patterns=CURIA_REQUIRED_FILES,
        force_download=bool(force),
    )
    root = Path(snapshot).resolve()
    health = curia_root_health(root)
    if not health.get("ok", False):
        raise RuntimeError(
            "Downloaded Curia snapshot failed verification:\n"
            + json.dumps(health, indent=2, default=str)
        )
    model_sha, _cfg = validate_curia_identity(root)
    log(
        f"[Curia] verified download: {root / 'model.safetensors'} "
        f"({(root / 'model.safetensors').stat().st_size / (1024**2):.1f} MiB, "
        f"sha256={model_sha})"
    )
    return root


def resolve_curia_root(
    allow_download: bool = True, force_download: bool = False
) -> Path:
    """Resolve real Curia weights, repairing tiny LFS pointers on local machines."""
    ...
    if root is not None:
        ...
        if health.get("ok", False):
            return root
        pointer = health.get("pointer") or {}
        if pointer:
            log(
                "[Curia] detected pointer file instead of weights: "
                f"{root / 'model.safetensors'}; "
                f"pointer oid={pointer.get('oid_sha256')}, "
                f"remote_size={pointer.get('remote_size_bytes')}"
            )
        else:
            log(f"[Curia] local asset verification failed: {health.get('reason')}")

    # Prefer an already prepared native-filesystem snapshot before touching the network.
    if CURIA_DOWNLOAD_ROOT.exists():
        cached_health = curia_root_health(CURIA_DOWNLOAD_ROOT)
        if cached_health.get("ok", False):
            log(f"[Curia] using verified prepared snapshot: {CURIA_DOWNLOAD_ROOT}")
            return CURIA_DOWNLOAD_ROOT.resolve()

    if allow_download and _curia_auto_download_allowed():
        return download_curia_snapshot(force=force_download)

    detail = (
        f"Local Curia root: {root}\nHealth: {json.dumps(curia_root_health(root), indent=2, default=str)}"
        if root is not None
        else f"Discovery error: {discovery_error!r}"
    )
    kaggle_note = (
        "\nKaggle hidden reruns are offline: attach a Kaggle Dataset containing the real "
        "Curia-2 files and set W6_CURIA_ROOT to that folder."
        if IS_KAGGLE
        else "\nRun `python rsna_w6_curia_teacher_spatial.py prepare_curia --accelerator localGPU` "
        "or set W6_CURIA_AUTO_DOWNLOAD=1."
    )
    raise RuntimeError(
        "No verified Curia-2 weight file is available.\n" + detail + kaggle_note
    )


def prepare_curia(
    accelerator: Optional[str] = None, force: bool = False
) -> Dict[str, Any]:
    """Explicitly repair/download Curia assets; useful before submit/cache_w61."""
    _ = resolve_runtime(accelerator)
    root = download_curia_snapshot(force=force)
    result = curia_environment_diagnostics(root)
    result["health"] = curia_root_health(root)
    result["resolved_root"] = str(root)
    result["pin"] = {"repo_id": CURIA_REPO_ID, "revision": CURIA_REVISION}
...
# Hugging Face custom-code loading mutates a shared dynamic-module cache.
# W6 test-cache workers are threads in one process, so serialize the complete
# AutoImageProcessor dynamic-module resolution/initialization step. Curia model
# inference remains parallel across GPUs after processor initialization.
_CURIA_PROCESSOR_LOAD_LOCK = threading.Lock()


def _load_curia_processor_serialized(curia_root: Path):
    try:
        from transformers import AutoImageProcessor
    except Exception as exc:
        raise RuntimeError("transformers is required for Curia-2.") from exc

    with _CURIA_PROCESSOR_LOAD_LOCK:
...
def _prewarm_curia_processor(curia_root: Path) -> None:
    """Load Curia custom processor once before multi-thread GPU workers start."""
    processor = _load_curia_processor_serialized(curia_root)
    # Validate the expected callable interface before launching expensive workers.
    if not callable(processor):
        raise RuntimeError(f"Curia processor is not callable: {type(processor)}")
    del processor
    gc.collect()
...
def load_curia_processor_and_model(curia_root: Path, device: torch.device):
    ...
    # Serialize custom dynamic-module initialization because test-cache workers
    # run concurrently in threads and share Transformers' module cache.
    processor = _load_curia_processor_serialized(curia_root)
...
def encode_study(
    ...
    payload = {
        "cache_version": EXPECTED_W4_CACHE_VERSION,
        "study_uid": study_uid,
...
def _parse_gpu_ids(text: str, env_name: str, count: int) -> List[int]:
    ids: List[int] = []
    for token in str(text or "").split(","):
        ...
        if value < 0 or value >= count:
            raise ValueError(
                f"{env_name} contains unavailable GPU {value}; cuda device count={count}"
            )
        if value not in ids:
            ids.append(value)
    return ids
...
def _default_cuda_ids(runtime: Runtime) -> List[int]:
    if runtime.accelerator != "cuda":
        return []
    count = int(torch.cuda.device_count())
    if count <= 0:
        return []
    # Kaggle GPU notebooks expose the two T4s to one process. Use both by
    # default. Local machines default to the explicitly selected device.
...
def cache_gpu_ids(runtime: Runtime) -> List[int]:
    """GPU IDs used for Curia feature extraction."""
    if runtime.accelerator != "cuda":
        return []
    count = int(torch.cuda.device_count())
    ids = (
        _parse_gpu_ids(CACHE_GPU_IDS_TEXT, "W6_CACHE_GPU_IDS", count)
        if CACHE_GPU_IDS_TEXT
        else []
    )
    return ids or _default_cuda_ids(runtime)


def training_gpu_ids(runtime: Runtime) -> List[int]:
    """GPU IDs used for independent CV/full-fit training jobs."""
    if runtime.accelerator != "cuda":
        return []
    count = int(torch.cuda.device_count())
    ids = (
        _parse_gpu_ids(TRAIN_GPU_IDS_TEXT, "W6_TRAIN_GPU_IDS", count)
        if TRAIN_GPU_IDS_TEXT
        else []
    )
    return ids or _default_cuda_ids(runtime)


def inference_gpu_ids(runtime: Runtime) -> List[int]:
    """GPU IDs used for checkpoint inference/ensembling."""
    if runtime.accelerator != "cuda":
        return []
    count = int(torch.cuda.device_count())
    ids = (
        _parse_gpu_ids(INFER_GPU_IDS_TEXT, "W6_INFER_GPU_IDS", count)
        if INFER_GPU_IDS_TEXT
        else []
    )
    return ids or _default_cuda_ids(runtime)


def runtime_for_cuda_gpu(gpu_id: int) -> Runtime:
    """Build a CUDA Runtime pinned to one device without changing global config."""
...
def _load_curia(runtime: Runtime):
    curia_root = resolve_curia_root(allow_download=True)
...
class TestFeatureStore(FeatureStore):
    """FeatureStore backed by submission-time test caches.

    W61 test caches contain both CLS features and patch grids, so W60 inference
    may safely fall back to the W61 test cache when an exact W60 cache is absent.
    """
    ...
    def __init__(self, uids: Sequence[str], variant: str):
        super().__init__(
            uids,
            variant,
            external_w4_cache=None,
            lru_size=FEATURE_LRU_SIZE,
        )
...
def _training_gpu_worker(
    ...
    """Persistent one-process-per-GPU worker for independent fold/seed jobs.

    A worker owns exactly one CUDA device and processes its assigned queue
    serially. This prevents two folds from accidentally landing on the same T4
    while another GPU is idle, and it avoids DDP synchronization overhead for
    these small cached-feature heads.
    """
    report: Dict[str, Any] = {
        "gpu_id": int(gpu_id),
    ...
    try:
        ...
        runtime = runtime_for_cuda_gpu(int(gpu_id))
        train_df, _series_df, gold_df, fold_zero = load_train_tables(
            require_dicom=False
        )
        teacher = load_teacher(train_df, strict=True)
        # Cache completeness is validated once in the parent before workers
        # launch. Each process gets its own disk-backed LRU to avoid cross-thread
        # mutable state and to keep W6.1 RAM bounded.
        external = discover_external_w4_cache() if variant == "w60" else None
        store = FeatureStore(
            train_df[UID].astype(str).tolist(),
            variant,
            external_w4_cache=external,
            lru_size=FEATURE_LRU_SIZE,
        )
        ...
        if job_kind == "cv":
            ...
            for fold in jobs:
                ...
                model, hist, _pred_df = train_model(
                    variant,
                    f"cv_fold_{fold}",
                    seed,
                    store,
            ...
            raise ValueError(f"Unknown training worker kind {job_kind!r}")
    except Exception as exc:
...
def _parallel_training_reports(
    variant: str,
    job_kind: str,
    jobs: Sequence[int],
    gpu_ids: Sequence[int],
) -> List[Dict[str, Any]]:
    """Launch one persistent spawned process per GPU and return worker reports.

    Parallel CUDA training must be launched from this file as a real Python
    script. Kaggle/IPython notebook execution via exec(), %run, or a directly
    pasted cell gives multiprocessing.spawn a synthetic ``__main__`` module;
    the child process then cannot unpickle ``_training_gpu_worker``. Detect that
    situation before launching so we fail immediately instead of appearing to
    hang.
    """
    gpu_ids = [int(x) for x in gpu_ids]
    jobs = [int(x) for x in jobs]
    if not gpu_ids:
        raise RuntimeError("Parallel CUDA training requested without GPU IDs")
    shards = [jobs[i :: len(gpu_ids)] for i in range(len(gpu_ids))]
    active = [(gpu_id, shard) for gpu_id, shard in zip(gpu_ids, shards) if shard]
    if len(active) <= 1:
        raise RuntimeError(
            "_parallel_training_reports requires at least two active GPU workers"
        )

    # multiprocessing.spawn must be able to reconstruct __main__ from a real
    # file. Kaggle notebook kernels expose an ipykernel-backed __main__, which
    # cannot provide this worker function to spawned children.
    main_mod = sys.modules.get("__main__")
    main_file = str(getattr(main_mod, "__file__", "") or "")
    main_loader = type(getattr(main_mod, "__loader__", None)).__name__
    interactive_main = (
        not main_file
        or "ipykernel" in main_file.lower()
        or "<stdin>" in main_file.lower()
        or main_file in {"-c", "<string>"}
    )
    if interactive_main:
        script_hint = (
            str(Path(__file__).resolve())
            if "__file__" in globals()
            else "rsna_w6_curia_teacher_spatial.py"
        )
        raise RuntimeError(
            "Parallel multi-GPU training was invoked from an interactive Kaggle/IPython "
            f"__main__ (file={main_file!r}, loader={main_loader}). Python spawn cannot "
            "re-import _training_gpu_worker from that notebook namespace. Run the same "
            "mode as a real OS Python process instead, for example:\n"
            f"  !python {script_hint} train_{variant}_{job_kind} --accelerator kaggle_t4\n"
            "Do not call run_w6(), exec(...), or %run for parallel training."
        )
    ...
    processes = []
    log(
        f"[{variant.upper()} {job_kind}] parallel GPU queues: "
        + ", ".join(f"cuda:{gpu_id}->{shard}" for gpu_id, shard in active)
    )
    for gpu_id, shard in active:
        process = ctx.Process(
            target=_training_gpu_worker,
            args=(variant, job_kind, shard, gpu_id, result_queue),
            daemon=False,
        )
        process.start()
        processes.append(process)

    # Fail fast. The previous implementation waited up to 24 hours on queue.get
    # even if every child had already crashed before posting a report.
    reports: List[Dict[str, Any]] = []
    expected_reports = len(processes)
    try:
        while len(reports) < expected_reports:
            try:
                reports.append(result_queue.get(timeout=5.0))
                continue
            ...
            if failed:
                ...
                raise RuntimeError(
                    f"Parallel {variant} {job_kind} worker exited before reporting; "
                    f"process_exitcodes={exitcodes}. Check the child traceback above."
                )

            # All workers exited cleanly but one or more reports never reached
            # the parent: this is also a terminal condition, not a 24h wait.
            if (
                all(not p.is_alive() for p in processes)
                and len(reports) < expected_reports
            ):
                raise RuntimeError(
                    f"Parallel {variant} {job_kind} workers exited but only "
                    f"{len(reports)}/{expected_reports} reports were received."
                )
    except Exception:
        for p in processes:
            if p.is_alive():
    ...
    process_failures = [p for p in processes if p.exitcode not in (0, None)]
    errors = [r for r in reports if r.get("error")]
    if process_failures or errors:
        details = {
            "process_exitcodes": [p.exitcode for p in processes],
            "worker_errors": errors,
        }
        raise RuntimeError(
            f"Parallel {variant} {job_kind} training failed: {json.dumps(details, indent=2)}"
        )
    return reports


def _aggregate_cv_results(variant: str) -> Dict[str, Any]:
    root = CHECKPOINT_ROOT / variant / "cv"
    result_root = RESULT_ROOT / variant / "cv"
    ...
    for fold in range(1, NUM_FOLDS + 1):
        history_path = result_root / f"fold_{fold}_history.csv"
        ckpt = root / f"fold_{fold}_epoch_{HEAD_EPOCHS}.pt"
        pred_path = ckpt.with_name(ckpt.stem + "_diagnostic_predictions.csv")
        if not ckpt.exists() or not history_path.exists():
            raise RuntimeError(
                f"Missing completed fold artifact for fold {fold}: {ckpt} / {history_path}"
            )
...
def train_cv(variant: str, accelerator: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Validate teacher/cache once before starting expensive workers.
    _teacher = load_teacher(train_df, strict=True)
    validation_store = _store_for_variant(train_df, variant)
    del validation_store, _teacher
    ...
    gpu_ids = training_gpu_ids(runtime) if runtime.accelerator == "cuda" else []
    use_parallel = (
        PARALLEL_TRAINING and runtime.accelerator == "cuda" and len(gpu_ids) >= 2
    )
    worker_reports: List[Dict[str, Any]] = []

    if use_parallel:
        worker_reports = _parallel_training_reports(
            variant, "cv", list(range(1, NUM_FOLDS + 1)), gpu_ids
        )
    else:
        teacher = load_teacher(train_df, strict=True)
        ...
        for fold in range(1, NUM_FOLDS + 1):
            ...
            ckpt = root / f"fold_{fold}_epoch_{HEAD_EPOCHS}.pt"
            history = result_root / f"fold_{fold}_history.csv"
            model, _hist, _pred_df = train_model(
                variant,
                f"cv_fold_{fold}",
                seed,
                store,
    ...
    summary = _aggregate_cv_results(variant)
    summary["parallel_training"] = bool(use_parallel)
    summary["training_gpu_ids"] = gpu_ids if gpu_ids else [str(runtime.device)]
    if worker_reports:
        summary["worker_reports"] = worker_reports
...
def train_full(variant: str, accelerator: Optional[str] = None) -> Dict[str, Any]:
    ...
    use_parallel = (
        PARALLEL_TRAINING
        and runtime.accelerator == "cuda"
        and len(gpu_ids) >= 2
        and len(FULLFIT_SEEDS) >= 2
    )
    worker_reports: List[Dict[str, Any]] = []

    if use_parallel:
        worker_reports = _parallel_training_reports(
            variant, "full", FULLFIT_SEEDS, gpu_ids
        )
    else:
        teacher = load_teacher(train_df, strict=True)
        store = _store_for_variant(train_df, variant)
    ...
    for seed in FULLFIT_SEEDS:
        ckpt = root / f"seed_{seed}_epoch_{HEAD_EPOCHS}.pt"
        history = result_root / f"seed_{seed}_history.csv"
        if not ckpt.exists() or not history.exists():
            raise RuntimeError(
                f"Missing full-fit artifact for seed {seed}: {ckpt} / {history}"
            )
    ...
    summary = {
        ...
        "seeds": FULLFIT_SEEDS,
        "models": completed,
        "parallel_training": bool(use_parallel),
        "training_gpu_ids": gpu_ids if gpu_ids else [str(runtime.device)],
...
def build_test_cache(variant: str, accelerator: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Transformers loads Curia's custom image processor through its dynamic-module
    # machinery. Prime that module once in the parent thread before two T4 worker
    # threads enter the loader concurrently. The workers still own separate Curia
    # model/processor instances and perform encoding simultaneously on both GPUs.
    if len(worker_devices) > 1:
        log(
            f"[test cache {variant}] Prewarming Curia image processor serially before GPU workers"
        )
        _prewarm_curia_processor(curia_root)
        log(f"[test cache {variant}] Curia image processor prewarm OK")
...
def _checkpoint_inference_worker(
    worker_id: int,
    gpu_id: int,
    assignments: Sequence[Tuple[str, str]],
    test_uids: Sequence[str],
) -> Dict[str, Any]:
    """Run an independent checkpoint queue on one GPU for submission inference."""
    torch.cuda.set_device(int(gpu_id))
    runtime = runtime_for_cuda_gpu(int(gpu_id))
    indices = np.arange(len(test_uids), dtype=np.int64)
    stores: Dict[str, TestFeatureStore] = {}
    outputs: List[Dict[str, Any]] = []
    log(
        f"[submission worker {worker_id}] device=cuda:{gpu_id} models={len(assignments)}"
    )
    for variant, ckpt_text in assignments:
        ckpt_path = Path(ckpt_text)
        if variant not in stores:
            stores[variant] = TestFeatureStore(test_uids, variant)
        ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        model = make_model(variant, runtime.device)
        model.load_state_dict(ckpt["model_state_dict"], strict=True)
        pred = predict_indices(
            model, stores[variant], indices, VALIDATION_BATCH_SIZE, runtime
        )
        outputs.append(
            {
                "model_id": f"{variant}:{ckpt_path.name}",
                "variant": variant,
        ...
        gc.collect()
        torch.cuda.empty_cache()
    return {"worker_id": worker_id, "gpu_id": gpu_id, "outputs": outputs}
...
def submit(accelerator: Optional[str] = None) -> Path:
    ...
    for variant in wanted:
        ...
        if not checkpoints:
            ...
            continue
        assignments.extend((variant, str(path)) for path in checkpoints)

    if not assignments:
        raise RuntimeError("No full-fit W60/W61 models are available for submission")

    infer_ids = inference_gpu_ids(runtime) if runtime.accelerator == "cuda" else []
    ...
    inference_workers: List[Dict[str, Any]] = []

    if use_parallel_inference:
        shards = [assignments[i :: len(infer_ids)] for i in range(len(infer_ids))]
        active = [(gpu_id, shard) for gpu_id, shard in zip(infer_ids, shards) if shard]
        log(
            "[submission] parallel model inference: "
            + ", ".join(
                f"cuda:{gpu_id}->{[Path(x[1]).name for x in shard]}"
                for gpu_id, shard in active
            )
        )
        with ThreadPoolExecutor(max_workers=len(active)) as executor:
            futures = [
                executor.submit(
                    _checkpoint_inference_worker, worker_id, gpu_id, shard, test_uids
                )
                for worker_id, (gpu_id, shard) in enumerate(active)
            ]
            for future in as_completed(futures):
                result = future.result()
                inference_workers.append(
                    {
                        "worker_id": result["worker_id"],
                        "gpu_id": result["gpu_id"],
                        "models": [x["model_id"] for x in result["outputs"]],
                    }
                )
                for item in result["outputs"]:
                    model_ids.append(item["model_id"])
                    model_predictions.append(item["prediction"])
    else:
        store_by_variant: Dict[str, TestFeatureStore] = {}
        indices = np.arange(len(test_df), dtype=np.int64)
        for variant, ckpt_text in assignments:
            ckpt_path = Path(ckpt_text)
            if variant not in store_by_variant:
                store_by_variant[variant] = TestFeatureStore(test_uids, variant)
            ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
            model = make_model(variant, runtime.device)
            model.load_state_dict(ckpt["model_state_dict"], strict=True)
            pred = predict_indices(
                model,
                store_by_variant[variant],
                indices,
    ...
    # Kaggle notebook competitions expect the rerun artifact at this exact path.
    # Keep descriptive files only for local experiments.
    path = (
        Path("/kaggle/working/submission.csv")
        if IS_KAGGLE
        else (SUBMISSION_ROOT / f"rsna_w6_{tag}_{FINAL_ENSEMBLE_MODE}_submission.csv")
    )
    path.parent.mkdir(parents=True, exist_ok=True)
    pred_df.to_csv(path, index=False)
    if IS_KAGGLE and not Path("/kaggle/working/submission.csv").exists():
        raise RuntimeError(
            "Kaggle submission.csv was not created at /kaggle/working/submission.csv"
        )
    summary = {
        "created_at": now_iso(),
        "submission": str(path),
        ...
        "parallel_inference": bool(use_parallel_inference),
        "inference_gpu_ids": infer_ids if infer_ids else [str(runtime.device)],
...
def check_curia(accelerator: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "auto_download_allowed": _curia_auto_download_allowed(),
            "download_root": str(CURIA_DOWNLOAD_ROOT),
            "pin": {"repo_id": CURIA_REPO_ID, "revision": CURIA_REVISION},
        }
        log(json.dumps(result, indent=2, default=str))
        return result
    ...
    result.update(
        {
            ...
            "expected_model_sha256": EXPECTED_CURIA_MODEL_SHA256,
            "auto_download_allowed": _curia_auto_download_allowed(),
            "download_root": str(CURIA_DOWNLOAD_ROOT),
            "pin": {"repo_id": CURIA_REPO_ID, "revision": CURIA_REVISION},
        }
    ...
    if pointer:
        result["diagnosis"] = (
            "The local model.safetensors is a Git-LFS/Xet pointer file, not the actual "
            "Curia weights. Run prepare_curia or allow automatic download."
        )
        result["pointer_expected_oid_matches"] = (
            pointer.get("oid_sha256") == EXPECTED_CURIA_MODEL_SHA256
        )
    elif health.get("ok", False):
        try:
            model_sha, config = validate_curia_identity(root)
            result["identity_sha256"] = model_sha
            result["identity_config"] = config
        except Exception as exc:
            result["identity_error"] = repr(exc)
    log(json.dumps(result, indent=2, default=str))
    return result
...
def status(accelerator: Optional[str] = None) -> Dict[str, Any]:
    ...
    info: Dict[str, Any] = {
        ...
        "training_gpu_ids": (
            training_gpu_ids(runtime) if runtime.accelerator == "cuda" else []
        ),
        "inference_gpu_ids": (
            inference_gpu_ids(runtime) if runtime.accelerator == "cuda" else []
        ),
    ...
    probe("curia_root", lambda: resolve_curia_root(allow_download=False))
    for name, path in (
        ("train_csv", TRAIN_CSV),
...
def run_w6(mode: str, accelerator: Optional[str] = None):
    ...
    if mode == "prepare_curia":
        return prepare_curia(accelerator)
    ...
    raise ValueError(
        f"Unknown mode {mode!r}. Expected status, check_curia, prepare_curia, validate_teacher, cache_w60, "
