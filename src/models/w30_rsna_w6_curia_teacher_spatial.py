...
EXPECTED_CURIA_MODEL_SHA256 = (
    ...
    # "403a02e27531d2858ecd1e9b1ec2d5ea" "7bfa909f10ff0d9e8416090a6a8c96ef",
    # "099c01a1af85ec3e2e9272608211ca74" "74165a0ab951e132b776d57f9dc1dffd",
    "099c01a1af85ec3e2e9272608211ca7474165a0ab951e132b776d57f9dc1dffd"
)
...
def cache_w61(accelerator: Optional[str] = None) -> Dict[str, Any]:
    ...
    curia_root, model_sha, config, processor, base_model = _load_curia(runtime)
    capture_model = (
        CuriaPatchCapture(base_model, SPATIAL_GRID).to(runtime.device).eval()
    )
    for p in capture_model.parameters():
    ...
    for idx, uid in enumerate(all_uids, 1):
        ...
        try:
            ...
            if captured is None or captured.shape != (
                expected_slices,
                SPATIAL_TOKENS,
                EXPECTED_CURIA_HIDDEN,
            ):
            ...
            s, k, _h = payload["features"].shape
            patch_grid = torch.zeros(
                s, k, SPATIAL_TOKENS, EXPECTED_CURIA_HIDDEN, dtype=torch.float16
            )
            cursor = 0
            for si in range(s):
                for ki in range(k):
                    if bool(payload["slice_mask"][si, ki]):
                        patch_grid[si, ki] = captured[cursor]
                        cursor += 1
            ...
            failures.append({UID: uid, "error": repr(exc)})
            log(f"[W61 cache] ERROR {uid}: {exc}")

        if idx % 25 == 0 or idx == len(all_uids):
            elapsed = max(time.time() - started, 1e-6)
            rate = encoded / elapsed
            projected = (len(all_uids) - existing) / max(rate, 1e-9)
            log(
                f"[W61 cache] {idx}/{len(all_uids)} new={encoded} "
                f"rate={rate:.3f} studies/s projected_new={projected/3600:.2f}h"
            )
    ...
    summary = {
        ...
        "created_at": now_iso(),
        "spatial_grid": SPATIAL_GRID,
        "spatial_tokens_per_slice": SPATIAL_TOKENS,
        "newly_encoded": encoded,
        ...
        "curia_sha256": model_sha,
        "curia_config": config,
        "warning": "W61 cache is intentionally larger than W4 because it preserves intra-slice patch regions.",
    }
    atomic_json_dump(summary, RESULT_ROOT / "20_w61_cache_summary.json")
    return summary
...
def build_test_cache(variant: str, accelerator: Optional[str] = None) -> Dict[str, Any]:
    ...
    curia_root, model_sha, config, processor, base_model = _load_curia(runtime)
    if variant == "w61":
        model = CuriaPatchCapture(base_model, SPATIAL_GRID).to(runtime.device).eval()
    else:
        model = base_model
    ...
    for i, uid in enumerate(uids, 1):
        ...
        if path.exists():
            try:
                ...
                okay = (
                    w61_payload_usable(p, uid)
                    if variant == "w61"
                    else w4_payload_usable(p, uid)
                )
                if okay:
                    continue
            except Exception:
        ...
        if variant == "w60":
            ...
            captured = torch.cat(model.captured, dim=0)
            expected = int(payload["slice_mask"].sum().item())
            if captured.shape[0] != expected:
                raise RuntimeError(
                    f"Spatial capture mismatch for {uid}: {captured.shape[0]} vs {expected}"
                )
            s, k, _h = payload["features"].shape
            pg = torch.zeros(
                s, k, SPATIAL_TOKENS, EXPECTED_CURIA_HIDDEN, dtype=torch.float16
            ...
            for si in range(s):
                for ki in range(k):
                    if bool(payload["slice_mask"][si, ki]):
                        pg[si, ki] = captured[cursor]
                        cursor += 1
            payload["w4_cache_version"] = payload.get("cache_version")
            payload["cache_version"] = SPATIAL_CACHE_VERSION
            payload["patch_grid"] = pg.contiguous()
            payload["patch_grid_shape"] = [SPATIAL_GRID, SPATIAL_GRID]
            if not w61_payload_usable(payload, uid):
        ...
        if i % 20 == 0 or i == len(uids):
            elapsed = max(time.time() - started, 1e-6)
            log(
                f"[{variant} test cache] {i}/{len(uids)} new={encoded} rate={encoded/elapsed:.3f} studies/s"
            )
...
def _cli() -> None:
    ...
    parser.add_argument(
        "--accelerator", default=None, help="auto/localGPU/apple_mps/cpu"
    )
    args = parser.parse_args()
    print(f"$env:W6_FINAL_VARIANTS='{FINAL_VARIANTS}'")
