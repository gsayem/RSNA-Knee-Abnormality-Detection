        ...
        json.dumps(
            {
                ...
                "semantic_engine": "dual_explicit_entailment_recovery_v2",
            },
            sort_keys=True,
        )
    )[:16]
...
def fit_stage_b_oof(
    ...
    for label in LABEL_COLUMNS:
        ...
        if evidence_n > 1:
            ...
            if len(np.unique(evidence_y)) == 2:

                evidence_auc = float(
                    roc_auc_score(
                        evidence_y,
                        oof_probability[evidence_mask],
                    )
                )
...
def apply_stage_b_calibrators(
    ...
    # Hard methodological gate:
    # no report evidence -> UNKNOWN, not a soft negative.
    output["SoftLabelAvailable"] = output["EvidenceAvailable"].astype(bool)
        ...
        coverage_rows.append(
            {
                ...
                "SemanticRecoveredN": int(
                    (group["FusionSource"] == "semantic_recovery").sum()
                ),
            }
        )
