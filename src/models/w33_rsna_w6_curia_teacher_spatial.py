...
def _parallel_training_reports(
    ...
    # Spawn is required: forking a process after CUDA has been touched can cause
    # "Cannot re-initialize CUDA in forked subprocess". Execute this file as a
    # normal Python script in Kaggle (e.g. !python ...), not as pasted cell code.
    ...
    for gpu_id, shard in active:
        process = ctx.Process(
            ...
            args=(variant, job_kind, shard, gpu_id, queue),
            daemon=False,
        )
        process.start()
    ...
    for _ in processes:
        try:
            reports.append(queue.get(timeout=24 * 3600))
        except Exception as exc:
            reports.append({"error": f"training worker result queue failure: {exc!r}"})
