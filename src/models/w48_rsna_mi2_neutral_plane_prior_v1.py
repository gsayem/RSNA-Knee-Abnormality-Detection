# %%writefile w48_rsna_mi2_neutral_plane_prior_v1.py
#!/usr/bin/env python3
"""
W48 | MedImageInsight Neutral Plane Prior v1
=============================================

Single-variable production ablation against W45/W47.

W48 removes the fixed hand-authored label-by-plane ATTENTION bias. Every prior
entry is 1.0. Since the attention code adds log(prior), W48 contributes exactly
zero fixed plane bias to every label and every token.

The learned plane embedding remains enabled. Therefore W48 does NOT remove MRI
plane metadata; it removes only the manually specified attention preference.
...
* W43 plane IDs: Axial=0, Coronal=1, Sagittal=2.
* Hidden dimension 192 and metadata dimensions.
* Learned plane embedding, label-aware attention, and pathology semantic pooling.
...
IMPORTANT
---------
W45/W47 head checkpoints MUST NOT be reused. W48 trains new lightweight heads
from the existing frozen W44 MI2 embedding cache under the neutral prior.

Standalone project rule:
* This file does not import/call/execute another project .py.
* No nn.DataParallel.
* Multiprocessing may spawn workers from this same standalone runtime.
"""
...
W48_SCRIPT_VERSION = "w48_mi2_neutral_plane_prior_v1"
W48_EXPERIMENT = "W48 | MedImageInsight Neutral Plane Prior v1"
W48_OUTPUT_DIR = "rsna_w48_mi2_neutral_plane_prior_v1"

# Internal compatibility aliases: the proven W47 v2 production functions are
# embedded below unchanged, but all persistent identities resolve to W48.
W47_SCRIPT_VERSION = W48_SCRIPT_VERSION
W47_EXPERIMENT = W48_EXPERIMENT
W47_OUTPUT_DIR = W48_OUTPUT_DIR
...
W47_MI2_BATCH = 4
W48_MI2_CACHE_VERSION = "w48_neutral_plane_mi2_test_embedding_cache_v1"
W47_MI2_CACHE_VERSION = W48_MI2_CACHE_VERSION
...
# W48 neutral fixed plane prior. Columns are [Axial, Coronal, Sagittal].
# log(1.0) == 0, therefore this matrix adds no manual attention bias.
W48_PLANE_PRIOR_ORDER = ["Axial", "Coronal", "Sagittal"]
W48_PLANE_PRIOR_MODE = "neutral_all_ones"
W48_PLANE_PRIOR = torch.ones((NUM_LABELS, 3), dtype=torch.float32)
W48_PLANE_PRIOR_LOG = torch.log(W48_PLANE_PRIOR.clamp_min(1e-3))

W47_PLANE_PRIOR_ORDER = W48_PLANE_PRIOR_ORDER
W47_PLANE_PRIOR = W48_PLANE_PRIOR
W47_PLANE_PRIOR_LOG = W48_PLANE_PRIOR_LOG
...
def _w47_mi2_cache_summary(
    cache_root: Path,
    sample_df_or_uids: Any,
) -> Dict[str, Any]:
    """Summarize hidden-test MI2 cache from a sample DataFrame or UID sequence."""
    if isinstance(sample_df_or_uids, pd.DataFrame):
        if UID_COLUMN not in sample_df_or_uids.columns:
            raise RuntimeError(f"MI2 cache summary DataFrame missing {UID_COLUMN}")
        uids = sample_df_or_uids[UID_COLUMN].astype(str).tolist()
    else:
        if isinstance(sample_df_or_uids, (str, bytes)):
            raise TypeError("MI2 cache summary expects a DataFrame or sequence of UIDs")
        uids = [str(uid) for uid in sample_df_or_uids]
...
def _w47_checkpoint_valid(path: Path, expected_seed: Optional[int] = None) -> bool:
    ...
    try:
        ...
        if payload.get("plane_prior_order") != W47_PLANE_PRIOR_ORDER:
            return False
        if payload.get("plane_prior_mode") != W48_PLANE_PRIOR_MODE:
            return False
        saved_prior = torch.as_tensor(payload.get("plane_prior"), dtype=torch.float32)
        if saved_prior.shape != W48_PLANE_PRIOR.shape or not torch.equal(
            saved_prior, W48_PLANE_PRIOR
        ):
            return False
...
def _w47_train_full(args) -> Dict[str, Any]:
    ...
    if not cache_summary["complete"]:
        raise RuntimeError(
            "W44 MI2 train cache is incomplete; do not rebuild it for W48. "
    ...
    _w47_log("-" * 100)
    _w47_log("STAGE P | W40 PSEUDO PRETRAIN | neutral plane prior")
    _w47_log("-" * 100)
    ...
    pseudo_payload = {
        ...
        "plane_prior_mode": W48_PLANE_PRIOR_MODE,
    ...
    for seed in PRODUCTION_SEEDS:
        _w47_log("-" * 100)
        _w47_log(f"FULL ALL-58 | seed={seed} | neutral plane prior")
        _w47_log("-" * 100)
    ...
    summary = {
        "script_version": W47_SCRIPT_VERSION,
        "experiment": W47_EXPERIMENT,
        "one_variable_change": "remove fixed hand-crafted plane-attention prior; keep learned plane embedding",
        "plane_ids": W47_W43_PLANE_TO_INDEX,
        "plane_prior_order": W47_PLANE_PRIOR_ORDER,
        "plane_prior_mode": W48_PLANE_PRIOR_MODE,
        "neutral_plane_prior": W47_PLANE_PRIOR.tolist(),
        "historical_w45_prior": W47_W45_HISTORICAL_PRIOR.tolist(),
        "pseudo_seed": PSEUDO_SEED,
...
# =============================================================================
# W48 HIDDEN TEST EXTRACTION / PREDICTION
# =============================================================================
...
def _w47_load_full_head(
    ...
    if payload.get("script_version") != W47_SCRIPT_VERSION:
        raise RuntimeError(f"W48 checkpoint version mismatch: {checkpoint}")
    if int(payload.get("seed", -1)) != int(seed):
        raise RuntimeError(f"W48 checkpoint seed mismatch: {checkpoint}")
    if payload.get("plane_prior_order") != W47_PLANE_PRIOR_ORDER:
        raise RuntimeError(f"W48 checkpoint plane prior order mismatch: {checkpoint}")
    if payload.get("plane_prior_mode") != W48_PLANE_PRIOR_MODE:
        raise RuntimeError(f"W48 checkpoint plane prior mode mismatch: {checkpoint}")
    saved_prior = torch.as_tensor(payload.get("plane_prior"), dtype=torch.float32)
    if saved_prior.shape != W48_PLANE_PRIOR.shape or not torch.equal(
        saved_prior, W48_PLANE_PRIOR
    ):
        raise RuntimeError(f"W48 checkpoint plane prior matrix mismatch: {checkpoint}")
    model = W47MI2Head()
...
def _w47_extract_test_mode(args) -> Dict[str, Any]:
    ...
    if str(cache_root).startswith("/kaggle/input/"):
        raise RuntimeError("W48 test cache must be writable under /kaggle/working")
...
def _w47_submit(args) -> Dict[str, Any]:
    ...
    saved_copy = result_root / "submission_w48_neutral_plane_mi2_5seed.csv"
    ...
    _w47_log("=" * 100)
    _w47_log("W48 MI2 SUBMISSION READY")
    _w47_log("=" * 100)
...
def _w47_status(args) -> Dict[str, Any]:
    ...
    prior_audit = {
        ...
        "runtime_plane_order": W47_PLANE_PRIOR_ORDER,
        "prior_mode": W48_PLANE_PRIOR_MODE,
        "historical_w45_matrix": W47_W45_HISTORICAL_PRIOR.tolist(),
        "w48_neutral_matrix": W47_PLANE_PRIOR.tolist(),
        "all_entries_are_one": bool(
            torch.equal(W47_PLANE_PRIOR, torch.ones_like(W47_PLANE_PRIOR))
        ),
        "all_log_prior_entries_are_zero": bool(
            torch.equal(W47_PLANE_PRIOR_LOG, torch.zeros_like(W47_PLANE_PRIOR_LOG))
        ),
        "learned_plane_embedding_enabled": True,
        "manual_plane_attention_bias_enabled": False,
    }
    ...
    payload = {
        "script_version": W47_SCRIPT_VERSION,
        "experiment": W47_EXPERIMENT,
        "one_variable_change": "remove fixed hand-crafted plane-attention prior only",
        "runtime": {
        ...
        "production_recipe": {
            ...
            "probability_ensemble": "mean",
            "calibration": "none",
            "plane_mapping": "neutral_all_ones_with_W43_Axial0_Coronal1_Sagittal2_metadata",
        },
...
def _w47_validate(args, raise_on_failure: bool = True) -> Dict[str, Any]:
    ...
    checks["neutral_plane_prior"] = (
        W47_PLANE_PRIOR_ORDER == ["Axial", "Coronal", "Sagittal"]
        and W47_W43_PLANE_TO_INDEX == {"Axial": 0, "Coronal": 1, "Sagittal": 2}
        and W48_PLANE_PRIOR_MODE == "neutral_all_ones"
        and torch.equal(W47_PLANE_PRIOR, torch.ones_like(W47_PLANE_PRIOR))
        and torch.equal(W47_PLANE_PRIOR_LOG, torch.zeros_like(W47_PLANE_PRIOR_LOG))
    )
...
def _w47_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="W48 standalone MedImageInsight neutral-plane-prior production submission"
    )
    parser.add_argument(
        "mode",
        nargs="?",
...
def w48_main() -> None:
    ...
    if mode == "status":
        ...
        if not status["ready_for_train_full"]:
            raise RuntimeError("W48 status is not ready for train_full")
        _w47_train_full(args)
        _w47_extract_test_mode(args)
        _w47_submit(args)
        _w47_validate(args, raise_on_failure=True)
