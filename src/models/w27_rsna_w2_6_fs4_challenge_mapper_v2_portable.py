...
# Reduce CUDA fragmentation during long sequential generation runs.
...
def _script_dir() -> Path:
    # Works both as a .py file and when pasted/executed in a notebook.
    try:
...
# Local layout requested by the user:
#   PROJECT_ROOT/
#     input/train.csv
#     output/results/rsna_w2/
#     output/results/rsna_w2_3/
#
# Override W26_PROJECT_ROOT if this script is not two levels below PROJECT_ROOT.
_DEFAULT_LOCAL_PROJECT_ROOT = (SCRIPT_DIR / ".." / "..").resolve()
PROJECT_ROOT = (
    Path(os.environ.get("W26_PROJECT_ROOT", str(_DEFAULT_LOCAL_PROJECT_ROOT)))
    .expanduser()
    .resolve()
)


def _first_existing(candidates: Sequence[Path], fallback: Path) -> Path:
    for p in candidates:
        if p.exists():
            return p
    return fallback
...
if IS_KAGGLE:
            ...
            os.environ.get(
                ...
                str(
                    _first_existing(
                        [
                            DATA_ROOT / "train.csv",
                            DATA_ROOT / "rsna-knee-abnormality-detection" / "train.csv",
                        ],
                        DATA_ROOT / "train.csv",
                    )
                ),
    ...
    _local_model_default = _first_existing(
        [
    ...
    OUTPUT_ROOT = (
        Path(
            os.environ.get(
                "W26_OUTPUT_ROOT",
                str(PROJECT_ROOT / "output" / "results" / "rsna_w2_6_v2"),
            )
        )
        .expanduser()
        .resolve()
...
#   auto        -> Kaggle T4 when on Kaggle CUDA, otherwise local_gpu on CUDA.
#   kaggle_t4   -> 2xT4 sharded FP16.
#   local_gpu   -> local CUDA; prefers 4-bit, otherwise FP16 + CPU offload.
#   tpu         -> recognized explicitly but intentionally not used for this
#                  Qwen2.5-7B generation path (see resolve_accelerator()).
ACCELERATOR = os.environ.get("W26_ACCELERATOR", "auto").strip().lower()
PRECISION = os.environ.get("W26_PRECISION", "auto").strip().lower()
GPU_ID = int(os.environ.get("W26_GPU_ID", "0"))

# Kaggle T4 memory policy.
T4_GPU_MAX_GIB = float(os.environ.get("W26_T4_GPU_MAX_GIB", "11.5"))

# Local 16GB CUDA card policy. In FP16 fallback, keep enough VRAM for KV cache.
LOCAL_GPU_MAX_GIB = float(os.environ.get("W26_LOCAL_GPU_MAX_GIB", "11.5"))
LOCAL_CPU_MAX_GIB = float(os.environ.get("W26_LOCAL_CPU_MAX_GIB", "36.0"))

# The original 6144-token prompt caused a ~632 MiB KV-cache allocation and
# OOM on a T4. V2 keeps the few-shot information but caps context much lower.
MAX_INPUT_TOKENS = int(os.environ.get("W26_MAX_INPUT_TOKENS", "3584"))
OOM_RETRY_TOKEN_BUDGETS = [
    int(x)
    for x in os.environ.get(
        "W26_OOM_RETRY_TOKEN_BUDGETS",
        "3072,2560,2048,1536",
    ).split(",")
    if x.strip()
]
MAX_NEW_TOKENS = int(os.environ.get("W26_MAX_NEW_TOKENS", "96"))

N_POS_EXAMPLES = int(os.environ.get("W26_N_POS_EXAMPLES", "2"))
N_NEG_EXAMPLES = int(os.environ.get("W26_N_NEG_EXAMPLES", "2"))

# Preserve both report head and tail via compact_report(); these limits keep
# nearly all ordinary reports intact while bounding worst-case prompt memory.
MAX_EXAMPLE_REPORT_CHARS = int(os.environ.get("W26_MAX_EXAMPLE_REPORT_CHARS", "1600"))
MAX_QUERY_REPORT_CHARS = int(os.environ.get("W26_MAX_QUERY_REPORT_CHARS", "2600"))
...
def build_mapper_prompt(
    ...
    chunks += [
        ...
        "",
        f"FINAL REMINDER — TARGET={label}: infer challenge label probability for the QUERY, "
        "using only fold-safe examples above. Query gold is unknown. Return JSON only.",
    ]
    return "\n".join(chunks)
...
# ============================================================
# 7. PORTABLE QWEN LOADER + OOM-RESILIENT GENERATION
# ============================================================
...
def resolve_accelerator(requested: Optional[str] = None) -> str:
    ...
    if req not in allowed:
        raise ValueError(
            f"Unknown W26 accelerator {req!r}. Use: auto, kaggle_t4, local_gpu, tpu, cpu"
        )
...
def _cuda_cleanup(torch_module) -> None:
    import gc
...
class LocalMapperLLM:
    def __init__(self, model_path: Path, accelerator: Optional[str] = None):
        ...
        if self.accelerator == "tpu":
            raise RuntimeError(
                "W26_ACCELERATOR='tpu' was selected, but this exact Qwen2.5-7B "
                "autoregressive mapper is intentionally NOT run through TPU/XLA. "
                "Multi-chip decoder generation would require a separate SPMD/FSDP "
                "implementation and would cost more time than this 232-call gate. "
                "Use local_gpu now; reserve TPU v5e-8 for the later image-model stage."
            )
        ...
        # Emergency token truncation should preserve the query and final output
        # contract at the END of the prompt rather than the oldest exemplar text.
        ...
        if self.accelerator == "kaggle_t4":
            ...
            if GPU_ID >= gpu_count:
                raise RuntimeError(
                    f"W26_GPU_ID={GPU_ID}, but only {gpu_count} CUDA GPUs are visible."
                )

            # On a 16GB 4060 Ti, 7B FP16 leaves too little activation headroom.
            # Prefer 4-bit when bitsandbytes is usable. If not, offload enough
            # FP16 weights to the user's 48GB system RAM.
            ...
            if try_4bit:
                try:
                    ...
                    if PRECISION in {"4bit", "nf4"}:
                        raise RuntimeError(
                            "Explicit local 4-bit loading failed. Install a compatible "
                            "bitsandbytes/transformers/accelerate stack, or use "
                            "W26_PRECISION=auto for FP16 CPU-offload fallback."
                        ) from exc
                    warnings.warn(
                        "Local 4-bit load unavailable; using FP16 + CPU offload. "
    ...
    def _generate_once(
        ...
        # Release allocator-reserved KV blocks from the previous query before
        # asking for the next large prefill.
        _cuda_cleanup(torch)
    ...
    def generate_text(
        ...
        """
        Returns:
          text, actual_input_tokens, token_budget_used

        OOM strategy is deterministic: retry the SAME prompt with progressively
        smaller context budgets. No sampling changes and no silent 0.5 fallback
        merely because one unusually long few-shot prompt exceeded T4 KV memory.
        """
        prompt = self._chat_prompt(user_prompt)

        budgets: List[int] = []
        for b in [MAX_INPUT_TOKENS, *OOM_RETRY_TOKEN_BUDGETS]:
            b = int(b)
        ...
        for attempt, budget in enumerate(budgets, start=1):
            try:
                raw, input_tokens = self._generate_once(prompt, budget)
                if attempt > 1:
                    log(
                        f"  OOM recovery succeeded at token_budget={budget} "
                        f"(input_tokens={input_tokens})"
                    )
                return raw, input_tokens, budget
            except torch.cuda.OutOfMemoryError as exc:
                last_oom = exc
                log(
                    f"  CUDA OOM at token_budget={budget}; "
                    f"retrying with a smaller context..."
                )
                _cuda_cleanup(torch)
                continue

        raise RuntimeError(
            "CUDA OOM at every W2.6 token budget: " + ",".join(str(x) for x in budgets)
        ) from last_oom
    ...
    def map_probability(
        ...
        raw = ""
        try:
            raw, input_tokens, token_budget = self.generate_text(user_prompt)
            parsed = parse_mapper_payload(extract_json_blob(raw))
            return parsed, raw, "ok", input_tokens, token_budget
        except self.torch.cuda.OutOfMemoryError:
            raise
        except RuntimeError as exc:
            if "CUDA OOM" in str(exc):
                raise
            first_exc = exc
        except Exception as exc:
            first_exc = exc

        # One JSON syntax/contract repair only. Semantic probabilities are not
        # repeatedly queried or cherry-picked.
        repair = (
            user_prompt
        ...
        try:
            raw2, input_tokens, token_budget = self.generate_text(repair)
            parsed = parse_mapper_payload(extract_json_blob(raw2))
            return parsed, raw2, "repaired", input_tokens, token_budget
        except RuntimeError as exc:
            if "CUDA OOM" in str(exc):
                raise
            log(f"  mapper parse fallback to 0.5: {exc}")
        except Exception as exc:
            log(f"  mapper parse fallback to 0.5: {exc}")
...
def cache_path() -> Path:
    return CACHE_ROOT / "w26_fs4_gold_oof_v2_portable.jsonl"
...
def run_gold_mapping(
    ...
    llm = LocalMapperLLM(model_path, accelerator=accelerator) if needed else None
    start = time.time()

    for i, q in enumerate(needed, start=1):
        t0 = time.time()
        parsed, raw, parse_status, input_tokens, token_budget = llm.map_probability(
            str(q["Prompt"])
        )
        row = {
            UID: str(q[UID]),
            ...
            "PromptVersion": "w26_fs4_fold_safe_fewshot_v2_portable",
        }
        append_cache(row)
...
def gold_w26(accelerator: Optional[str] = None) -> Dict[str, Any]:
    ensure_dirs()
    ...
    log("=" * 96)
    log("RSNA W2.6 V2 — PORTABLE OOM-SAFE FS4 CHALLENGE MAPPER")
    log("=" * 96)
    ...
    fs4 = run_gold_mapping(queries, model_path, accelerator=accelerator)
    fs4.to_csv(
    ...
    manifest = {
        "experiment": "RSNA W2.6 V2 portable OOM-safe FS4 challenge mapper",
        "version": "w26_fs4_fold_safe_fewshot_v2_portable",
...
def status_w26() -> Dict[str, Any]:
    payload: Dict[str, Any] = {
        ...
        "accelerator_resolved": resolve_accelerator(),
        ...
        "fixed_blend_alpha": FIXED_BLEND_ALPHA,
        "t4_gpu_max_gib": T4_GPU_MAX_GIB,
        "local_gpu_max_gib": LOCAL_GPU_MAX_GIB,
        "local_cpu_max_gib": LOCAL_CPU_MAX_GIB,
        "max_input_tokens": MAX_INPUT_TOKENS,
        "oom_retry_token_budgets": OOM_RETRY_TOKEN_BUDGETS,
        "max_new_tokens": MAX_NEW_TOKENS,
    }
    try:
...
def run_w26(
    ...
    if accelerator is not None:
        # Validate early even for status.
        accelerator = resolve_accelerator(accelerator)

    if mode == "status":
        payload = status_w26()
        if accelerator is not None:
            payload["accelerator_call_override"] = accelerator
            log(f"run_w26 accelerator override: {accelerator}")
        return payload
    if mode == "gold":
        return gold_w26(accelerator=accelerator)
    if mode == "validate":
        return validate_w26()
...
def main():
    ...
    # run_w26(args.mode)
    # run_w26("status")
    # run_w26("gold", accelerator="localGPU")
    run_w26("validate")
