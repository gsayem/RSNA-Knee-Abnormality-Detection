...
W43 — RadImageNet Inception-ResNet-v2 2.5D Multi-Plane v1
==========================================================================

PRIMARY HYPOTHESIS
------------------
Replace the frozen Curia CLS representation with a fully trainable medical-image
backbone initialized from RadImageNet Inception-ResNet-v2, while preserving W40
...
--------------------
    all MRI series
        ...
        -> 2.5D stacks
        -> RadImageNet Inception-ResNet-v2
...
If the official Inception-ResNet-v2 PyTorch checkpoint uses a different key
...
The cache is intentionally NOT named after Inception-ResNet-v2. W44
DenseNet-121 must reuse the exact same cache for a clean backbone comparison.

CLI
---
    python w43_rsna_radimagenet_irv2_2p5d_v1.py status --accelerator localGPU

    python w43_rsna_radimagenet_irv2_2p5d_v1.py cache \
        --accelerator localGPU --cache-workers 8

    python w43_rsna_radimagenet_irv2_2p5d_v1.py train_pseudo \
        --accelerator localGPU

    python w43_rsna_radimagenet_irv2_2p5d_v1.py train_cv \
        --accelerator localGPU

    python w43_rsna_radimagenet_irv2_2p5d_v1.py train_full \
        --accelerator localGPU

    python w43_rsna_radimagenet_irv2_2p5d_v1.py train_all \
        --accelerator localGPU

    python w43_rsna_radimagenet_irv2_2p5d_v1.py validate \
        --accelerator localGPU

Explicit RadImageNet weights:
    --radimagenet-weights /path/to/InceptionResNetV2.pt
...
SCRIPT_VERSION = "w43_radimagenet_irv2_2p5d_v1"
DISPLAY_VERSION = "W43 | RadImageNet Inception-ResNet-v2 2.5D Multi-Plane v1"
OUTPUT_DIR_NAME = "rsna_w43_radimagenet_irv2_2p5d_v1"
...
# =============================================================================
# 2.5D CACHE — SHARED WITH FUTURE W44 DENSENET
# =============================================================================
...
class ProjectPaths:
    ...
    def discover(cls, args) -> "ProjectPaths":
        ...
        if args.cache_root:
            ...
            cache_root = project_root / "output" / "results" / "rsna_2p5d_mri_cache_v1"
...
def discover_radimagenet_weights(project_root: Path) -> Optional[Path]:
    ...
    patterns = [
        "*Inception*ResNet*V2*.pt",
        "*Inception*ResNet*V2*.pth",
        "*inception*resnet*v2*.pt",
        "*inception*resnet*v2*.pth",
        "*inceptionresnetv2*.pt",
        "*inceptionresnetv2*.pth",
    ]
...
def run_cache(paths: ProjectPaths, args) -> Dict[str, Any]:
    ...
    log("Cache is backbone-independent and reusable by W44 DenseNet-121.")
...
# =============================================================================
# RADIMAGENET INCEPTION-RESNET-v2 IDENTITY / LOAD
# =============================================================================
...
def require_timm():
    try:
        ...
        raise RuntimeError(
            "W43 requires timm. Install it with `pip install timm` in the environment."
        ) from exc
    return timm


def create_irv2_backbone() -> nn.Module:
    timm = require_timm()
    ...
    feature_dim = int(getattr(model, "num_features", BACKBONE_FEATURE_DIM))
    if feature_dim != BACKBONE_FEATURE_DIM:
        raise RuntimeError(
            f"Unexpected {BACKBONE_NAME} num_features={feature_dim}; expected={BACKBONE_FEATURE_DIM}"
        )
    return model
...
def _extract_checkpoint_state(payload: Any) -> Dict[str, torch.Tensor]:
    ...
    raise RuntimeError(
        "Could not identify a PyTorch state_dict inside the RadImageNet checkpoint."
    )
...
def audit_radimagenet_weights(path: Path) -> Dict[str, Any]:
    ...
    backbone = create_irv2_backbone()
    model_state = backbone.state_dict()

    payload = torch.load(path, map_location="cpu", weights_only=False)
    raw_state = _extract_checkpoint_state(payload)
    checkpoint_state = {
        _normalize_checkpoint_key(key): value for key, value in raw_state.items()
    }
    ...
    for key, model_tensor in model_state.items():
        ...
        if checkpoint_tensor is None:
            # Older checkpoints may omit this BatchNorm bookkeeping buffer.
    ...
    unexpected = [
        ...
        if key not in model_state and not key.startswith(ignored_classifier_prefixes)
    ]
    ...
    audit = {
        ...
        "checkpoint_state_keys": int(len(checkpoint_state)),
        ...
        "allowed_missing_bn_tracking_count": int(len(allowed_missing)),
        ...
        "checkpoint_first_keys": list(checkpoint_state.keys())[:20],
...
def load_radimagenet_backbone(path: Path) -> Tuple[nn.Module, Dict[str, Any]]:
    ...
    if not audit["overall_pass"]:
        raise RuntimeError(
            "RadImageNet Inception-ResNet-v2 checkpoint audit FAILED. "
    ...
    loadable = {
        ...
        and tuple(checkpoint_state[key].shape) == tuple(tensor.shape)
    }

    result = backbone.load_state_dict(loadable, strict=False)
    disallowed_missing = [
    ...
    if disallowed_missing:
        raise RuntimeError(
            f"RadImageNet backbone load left unmatched tensors: {disallowed_missing[:20]}"
        )

    # Unexpected keys cannot exist because loadable was intersected with model keys.
...
class RadImageNetIRV2StudyModel(nn.Module):
    ...
    def forward(
        ...
        if valid_features.ndim != 2 or valid_features.shape[-1] != BACKBONE_FEATURE_DIM:
            raise RuntimeError(
                f"Unexpected Inception-ResNet-v2 feature shape {tuple(valid_features.shape)}"
            )
...
def unwrap_model(model: nn.Module) -> RadImageNetIRV2StudyModel:
...
def set_backbone_train_stage(model: nn.Module, stage: str) -> None:
    ...
    if stage == "top":
        top_names = ("mixed_7a", "repeat_2", "block8", "conv2d_7b")
        found = 0
        for name, module in core.backbone.named_children():
        ...
        if found == 0:
            raise RuntimeError(
                "Could not identify top Inception-ResNet-v2 modules for progressive unfreezing."
            )
        return
...
def create_new_training_model(
    ...
    if paths.radimagenet_weights is None:
        raise RuntimeError(
            "RadImageNet Inception-ResNet-v2 weights were not discovered. "
            "Pass --radimagenet-weights explicitly."
        )

    backbone, audit = load_radimagenet_backbone(paths.radimagenet_weights)
    model = RadImageNetIRV2StudyModel(backbone).to(runtime.device)
...
def run_train_full(
    ...
    summary = {
        ...
        "teacher": "W40",
        "backbone": "RadImageNet Inception-ResNet-v2",
        "cache_version": CACHE_VERSION,
    }
...
def run_status(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    ...
    try:
        ...
        dependency["timm"] = True
    except Exception:
        dependency["timm"] = False
    ...
    rad_audit = None
    rad_error = None
    if paths.radimagenet_weights is not None and dependency.get("timm"):
        try:
        ...
        rad_error = (
            "RadImageNet Inception-ResNet-v2 checkpoint not discovered. "
            "Pass --radimagenet-weights /path/to/checkpoint.pt"
        )
    elif not dependency.get("timm"):
        rad_error = "timm is not installed."
    ...
    payload = {
        ...
        "ready_for_cache": bool(data_error is None and dependency["pydicom"]),
        "ready_for_training": bool(
            data_error is None
            and fold_error is None
            and teacher_error is None
            and dependency.get("timm")
