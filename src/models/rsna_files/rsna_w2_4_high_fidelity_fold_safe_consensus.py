...
EXPLICIT_W23_ROOT = os.environ.get('W24_W23_ROOT', '').strip()
EXPLICIT_PUBLIC_ROOT = os.environ.get('W24_PUBLIC_ROOT', '').strip()
EXPLICIT_PUBLIC_FILE = os.environ.get('W24_PUBLIC_FILE', '').strip()
...
SILENCE_NEGATIVE_WEIGHT_CAP = float(os.environ.get('W24_SILENCE_NEGATIVE_WEIGHT_CAP', '0.35'))
...
LABEL_ALIASES: Dict[str, List[str]] = {
    ...
    'PF OA': ['pfoa', 'patellofemoralOA', 'patellofemoralosteoarthritis', 'patellofemoral'],
...
UID_ALIASES = [
    'studyinstanceuid', 'studyuid', 'studyid', 'uid', 'id', 'study_instance_uid'
]
...
def bootstrap_macro_delta(
    ...
    if not deltas:
        return {'mean_delta': float('nan'), 'ci_low': float('nan'), 'ci_high': float('nan'), 'p_gt_0': float('nan'), 'n_bootstrap': 0}
...
def load_train() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    gold = (
        ...
        .copy().sort_values(UID_COLUMN).reset_index(drop=True)
    )
    unlabeled = (
    ...
    if len(train) != EXPECTED_TOTAL_STUDIES:
        raise RuntimeError(f'Expected {EXPECTED_TOTAL_STUDIES} train studies, found {len(train)}')
    if len(gold) != EXPECTED_GOLD_STUDIES:
        raise RuntimeError(f'Expected {EXPECTED_GOLD_STUDIES} gold studies, found {len(gold)}')
    if len(unlabeled) != EXPECTED_UNLABELED_STUDIES:
        raise RuntimeError(f'Expected {EXPECTED_UNLABELED_STUDIES} unlabeled studies, found {len(unlabeled)}')
...
def looks_like_w23_root(path: Path) -> bool:
    return (
        ...
        and all((path / 'folds' / f'fold_{f}' / 'soft_labels_long.csv').exists() for f in range(1, 6))
    )
...
def load_and_verify_fold_assignments(w23_root: Path, gold: pd.DataFrame) -> pd.DataFrame:
    ...
    fold_df = fold_df[[UID_COLUMN, 'OuterFold']].sort_values(UID_COLUMN).reset_index(drop=True)
...
def label_column_candidates(columns: Sequence[str], label: str) -> List[str]:
    aliases = [normalize_name(label), *[normalize_name(x) for x in LABEL_ALIASES[label]]]
    prefixes = ['', 'p', 'prob', 'probability', 'score', 'prediction', 'pred', 'label', 'llm', 'pseudo']
    ...
    for column in columns:
        ...
        for alias in aliases:
            for prefix_rank, prefix in enumerate(prefixes):
                ...
                elif n.endswith(alias) and any(n.startswith(p) for p in ['prob', 'score', 'pred', 'llm', 'pseudo']):
...
def confidence_column_candidates(columns: Sequence[str], label: str) -> List[str]:
    ...
    for column in columns:
        n = normalize_name(column)
        if any(alias in n for alias in aliases) and any(key in n for key in ['conf', 'confidence', 'weight', 'reliability']):
...
def wide_teacher_from_frame(frame: pd.DataFrame, name: str, path: Path) -> Optional[TeacherTable]:
...
def long_teacher_from_frame(frame: pd.DataFrame, name: str, path: Path) -> Optional[TeacherTable]:
    uid_col = find_uid_column(frame.columns)
    if uid_col is None:
        return None
    normalized = {normalize_name(c): c for c in frame.columns}
    label_col = next((normalized[k] for k in ['label', 'target', 'finding', 'condition'] if k in normalized), None)
    value_col = next((normalized[k] for k in ['probability', 'prob', 'score', 'prediction', 'pred', 'value'] if k in normalized), None)
    conf_col = next((normalized[k] for k in ['confidence', 'conf', 'weight', 'reliability'] if k in normalized), None)
    ...
    work = frame[[uid_col, label_col, value_col] + ([conf_col] if conf_col else [])].copy()
    work['_CanonicalLabel'] = work[label_col].map(lambda x: alias_to_label.get(normalize_name(x)))
    ...
    if work[[uid_col, '_CanonicalLabel']].duplicated().any():
        raise RuntimeError(f'{name}: duplicate UID/label rows in long teacher file {path}')
    probs = work.pivot(index=uid_col, columns='_CanonicalLabel', values='_Value').reset_index()
    ...
    if conf_col:
        ...
        conf_frame = work.pivot(index=uid_col, columns='_CanonicalLabel', values='_Conf').reset_index()
...
def score_teacher_candidate(path: Path) -> Tuple[int, Dict[str, Any]]:
    ...
    mapped = sum(bool(label_column_candidates(frame.columns, label)) for label in LABEL_COLUMNS)
    normalized = {normalize_name(c) for c in frame.columns}
    is_long = bool(uid and {'label'} & normalized and ({'probability', 'prob', 'score', 'prediction', 'pred', 'value'} & normalized))
    score = (50 if uid else 0) + mapped * 10 + (100 if mapped == 12 else 0) + (80 if is_long else 0)
    return score, {
        'uid_column': uid,
...
def discover_public_teacher_file() -> Tuple[Path, List[Dict[str, Any]]]:
    ...
    audit = [{'path': str(path), 'score': score, **info} for score, path, info in candidates]
...
def public_teacher_status() -> Dict[str, Any]:
    ...
    train_uid_set = set(gold[UID_COLUMN].astype(str)) | set(unlabeled[UID_COLUMN].astype(str))
...
def _ensure_canonical_long(frame: pd.DataFrame, required: Sequence[str], name: str) -> pd.DataFrame:
...
def assertion_numeric(value: Any) -> float:
    ...
    positive = {
        'positive', 'present', 'abnormal', 'yes', 'true', 'affirmed', 'likelypositive'
    }
    negative = {
        'negative', 'absent', 'normal', 'no', 'false', 'intact', 'likelynegative'
    }
    uncertain = {
        'uncertain', 'equivocal', 'possible', 'indeterminate', 'notmentioned',
        'noevidence', 'unknown', 'neutral', ''
    }
...
def load_w23_inner_train(w23_root: Path, fold: int) -> pd.DataFrame:
    path = w23_root / 'folds' / f'fold_{fold}' / 'stage_b_inner_gold_oof_predictions.csv'
    ...
    output = pd.DataFrame({
        ...
        'RawProbability': pd.to_numeric(frame['InnerOOFProbability'], errors='coerce'),
        'EvidenceAvailable': frame['EvidenceAvailable'],
        'FusedAssertion': frame['FusedAssertion'].map(assertion_numeric).astype(float),
...
def load_w23_unlabeled(w23_root: Path, fold: int) -> pd.DataFrame:
    ...
    output = pd.DataFrame({
        UID_COLUMN: frame[UID_COLUMN].astype(str),
        'Label': frame['Label'].astype(str),
        'RawProbability': pd.to_numeric(frame['ChallengeProbabilityRaw'], errors='coerce'),
        'EvidenceAvailable': frame['EvidenceAvailable'],
        'FusedAssertion': frame['FusedAssertion'].map(assertion_numeric).astype(float),
        'FusedAssertionConfidence': pd.to_numeric(
            frame['FusedAssertionConfidence'], errors='coerce'
...
def teacher_probability_for_uids(
    ...
    p = pd.to_numeric(probs.reindex(list(map(str, uids)))[label], errors='coerce').to_numpy(dtype=np.float64)
...
def fit_platt_calibrator(
    ...
    if train_n == 0 or len(np.unique(yv)) < 2:
        return PlattCalibrator(
            label, source, False, False, 1.0, 0.0,
            train_n, positive_n,
            float('nan'), float('nan'), float('nan'), float('nan'), QUALITY_FLOOR,
        )
...
def _teacher_conf_index(teacher: TeacherTable) -> Optional[pd.DataFrame]:
    return teacher.confidence.set_index(UID_COLUMN) if teacher.confidence is not None else None
...
def _teacher_scalar(
    ...
    if teacher.confidence is not None:
        ...
        c = safe_float(confs.at[uid, label]) if (confs is not None and uid in confs.index) else float('nan')
...
def fit_fold_calibrators(
    ...
    for label in LABEL_COLUMNS:
        ...
        subset = subset.set_index(UID_COLUMN).reindex(train_gold[UID_COLUMN].astype(str)).reset_index()
...
def _fuse_rows_for_population(
    ...
    teacher_prob_indices = {teacher.name: _teacher_index(teacher) for teacher in external_teachers}
    teacher_conf_indices = {teacher.name: _teacher_conf_index(teacher) for teacher in external_teachers}
    ...
    for uid in uids:
        for label in LABEL_COLUMNS:
            ...
            if key not in w23_lookup.index:
                raise RuntimeError(f'Missing W2.3 {population} row for {uid}/{label}/fold {fold}')
            wrow = w23_lookup.loc[key]
            if isinstance(wrow, pd.DataFrame):
                raise RuntimeError(f'Duplicate W2.3 {population} row for {uid}/{label}/fold {fold}')
            ...
            calibrated_w23 = float(cal_w23.predict(np.array([raw_w23]))[0]) if np.isfinite(raw_w23) else float('nan')
            ...
            for teacher in external_teachers:
                ...
                calibrated = float(cal.predict(np.array([raw]))[0]) if np.isfinite(raw) else float('nan')
            ...
            for teacher in external_teachers:
                ...
                row[f'{teacher.name}__ReliabilityQuality'] = calibrators[(teacher.name, label)].quality
    ...
    if result[[UID_COLUMN, 'Label']].duplicated().any():
        raise RuntimeError(f'Duplicate fused UID/Label rows in fold {fold}/{population}')
    return result
...
def write_fold_outputs(
    ...
    weight = long_to_wide(unlabeled_long, 'ConsensusWeight', unlabeled_uids, fill_value=0.0)
    availability = long_to_wide(unlabeled_long, 'SoftLabelAvailable', unlabeled_uids, fill_value=False)
    selection = long_to_wide(unlabeled_long, 'ConsensusWeight', unlabeled_uids, fill_value=0.0)
    agreement = long_to_wide(unlabeled_long, 'SourceAgreement', unlabeled_uids, fill_value=0.0)
...
def evaluate_oof_sources(
    ...
    comparison['W23_AP'] = [float(w23.at[label, 'AveragePrecision']) for label in LABEL_COLUMNS]
    comparison['W24_AP'] = [float(w24.at[label, 'AveragePrecision']) for label in LABEL_COLUMNS]
    ...
    comparison['Delta_Brier_W24MinusW23'] = comparison['W24_Brier'] - comparison['W23_Brier']

    macro = {
        source_name: {
            'macro_AUROC': float(np.nanmean(
                metrics_all.loc[metrics_all['Source'] == source_name, 'AUROC'].values
            )),
            'macro_AP': float(np.nanmean(
                metrics_all.loc[metrics_all['Source'] == source_name, 'AveragePrecision'].values
            )),
            'macro_Brier': float(np.nanmean(
                metrics_all.loc[metrics_all['Source'] == source_name, 'Brier'].values
            )),
        }
...
def summarize_unlabeled_coverage(unlabeled_parts: Sequence[pd.DataFrame]) -> pd.DataFrame:
...
def build_w24() -> Dict[str, Any]:
    ...
    for fold in range(1, NUM_OUTER_FOLDS + 1):
        ...
        if set(w23_unlabeled[UID_COLUMN].astype(str)) != set(unlabeled[UID_COLUMN].astype(str)):
    ...
    coverage.to_csv(
        RESULT_ROOT / '06_unlabeled_coverage.csv', index=False
    )
    ...
    comparison.to_csv(
        RESULT_ROOT / '05_w24_vs_w23_per_label.csv', index=False
    )
...
def validate_w24() -> Dict[str, Any]:
    ...
    if not manifest_path.exists() or not summary_path.exists() or not comparison_path.exists():
        raise FileNotFoundError('W2.4 build outputs not found. Run run_w24("build") first.')
    ...
    checks: Dict[str, Any] = {
        'fold_hash_match': manifest['outer_folds']['assignment_sha256'] == EXPECTED_FOLD_SHA256,
        'five_fold_output_dirs': all((FOLD_ROOT / f'fold_{f}').exists() for f in range(1, 6)),
    ...
    for fold in range(1, 6):
        ...
        if len(probability) != EXPECTED_UNLABELED_STUDIES or len(weight) != EXPECTED_UNLABELED_STUDIES:
            checks['all_fold_rows_4349'] = False
        p = probability[LABEL_COLUMNS].apply(pd.to_numeric, errors='coerce').to_numpy(dtype=float)
        w = weight[LABEL_COLUMNS].apply(pd.to_numeric, errors='coerce').to_numpy(dtype=float)
