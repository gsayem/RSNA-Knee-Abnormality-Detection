...
EXPLICIT_PUBLIC_FILE = os.environ.get(
    ...
    os.environ.get('W24_PUBLIC_FILE', ''),  # backward-compatible alias
).strip()
...
def confidence_column_candidates(columns: Sequence[str], label: str) -> List[str]:
    ...
    long_aliases = [
        normalize_name(x)
        for x in LABEL_ALIASES[label]
        if len(normalize_name(x)) >= 4
    ]
    ...
    if EXPLICIT_PUBLIC_FILE and EXPLICIT_TEACHER3_FILE:
        try:
            ...
            if primary_path.exists() and teacher3_path.exists():
                payload['primary_teacher_equals_teacher3'] = (
                    sha256_file(primary_path) == sha256_file(teacher3_path)
                )
        except Exception as exc:
