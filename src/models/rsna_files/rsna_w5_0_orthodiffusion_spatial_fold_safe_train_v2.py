...
# Exceptional repair path for studies that otherwise have zero usable series.
# IMPORTANT: CACHE_VERSION is intentionally unchanged so the 4,405 already
# completed study caches remain valid. The repair is deterministic, explicitly
# marked in repaired study payloads/audits, and affects only exceptional studies.
CACHE_REPAIR_VERSION = "w5_0_exceptional_study_repair_v1"
EXCEPTIONAL_MIN_DECODABLE_SLICES = 2
...
# ============================================================
# 7B. EXCEPTIONAL STUDY REPAIR (ONLY IF ZERO NORMAL SERIES SURVIVE)
# ============================================================


def prepare_exceptional_series_window(
    study_uid: str,
    series_uid: str,
    metadata_plane: str,
) -> Tuple[List[torch.Tensor], List[int], List[float], Dict[str, Any]]:
    """
    Deterministic emergency path used only when a study would otherwise have
    ZERO Ortho-eligible series.

    It decodes every usable slice in the series, resizes each to 256x256, then
    interpolates the physical slice stack along depth to 16 slices. This makes
    the final study representable without changing the normal 3-window policy
    used by the 4,405 already cached studies.

    This is not silently mixed into ordinary series: repaired series are marked
    Status=READY_REPAIR / OK_REPAIR and the study payload stores repair_version.
    """
    try:
        import cv2
    except Exception as exc:
    ...
    n_slices = len(records)
    if n_slices == 0:
        raise RuntimeError("Series contains zero DICOM slices")
    ...
    for record_index, record in enumerate(records):
        try:
            image = decode_rescaled_slice(record["path"])
            if image.shape != (ORTHO_INPUT_SIZE, ORTHO_INPUT_SIZE):
                image = cv2.resize(
                    image,
                    (ORTHO_INPUT_SIZE, ORTHO_INPUT_SIZE),
            ...
            if not np.isfinite(image).all():
                raise RuntimeError("decoded slice contains non-finite values")
            decoded.append(image.astype(np.float32, copy=False))
            decoded_indices.append(record_index)
        except Exception as exc:
            decode_errors.append(f"{record_index}:{type(exc).__name__}:{exc}")

    if len(decoded) < EXCEPTIONAL_MIN_DECODABLE_SLICES:
        raise RuntimeError(
            f"Only {len(decoded)}/{n_slices} slices were decodable; "
            f"need >= {EXCEPTIONAL_MIN_DECODABLE_SLICES}. "
            f"Errors={decode_errors[:3]}"
        )

    # [D,H,W] -> [1,1,D,H,W] and interpolate only the depth dimension;
    # H/W are already exactly 256x256. align_corners=False is deterministic.
    source = (
        torch.from_numpy(np.stack(decoded, axis=0)).unsqueeze(0).unsqueeze(0).float()
    )

    repaired = F.interpolate(
        source,
        size=(ORTHO_DEPTH_SIZE, ORTHO_INPUT_SIZE, ORTHO_INPUT_SIZE),
        mode="trilinear",
        align_corners=False,
    )[0, 0]

    # Official-style per-volume min/max normalization after the repair.
    minimum = float(repaired.min().item())
    maximum = float(repaired.max().item())
    repaired = (repaired - minimum) / (maximum - minimum + 1e-8)
    repaired = repaired * 2.0 - 1.0
    tensor = repaired.unsqueeze(0).contiguous().float()

    expected = (1, ORTHO_DEPTH_SIZE, ORTHO_INPUT_SIZE, ORTHO_INPUT_SIZE)
    if tuple(tensor.shape) != expected:
        raise RuntimeError(
            f"Exceptional repair produced {tuple(tensor.shape)} != {expected}"
        )
    if not torch.isfinite(tensor).all():
        raise RuntimeError("Exceptional repaired volume contains non-finite values")
    ...
    audit = {
        ...
        "WindowCount": 1,
        "WindowStarts": "-1",
        "WindowCenters": "0.000000",
        "StackSpanMM": physical_stack_span_mm(records),
        "PixelSpacingMeanMM": pixel_mean,
        "RepairVersion": CACHE_REPAIR_VERSION,
        "RepairReason": "study_had_zero_normal_eligible_series",
        "DecodedSliceCount": int(len(decoded)),
        "FailedDecodeCount": int(len(decode_errors)),
        "DecodedSourceIndices": "|".join(str(x) for x in decoded_indices),
        "RepairDecodeErrors": " || ".join(decode_errors[:10]),
    }

    # start=-1 is an explicit sentinel and becomes part of the stable diffusion
    # noise seed. center=0 means the repaired window represents the full stack.
    return [tensor], [-1], [0.0], audit
...
def encode_study(
    ...
    for _, row in ordered.iterrows():
        ...
        try:
            ...
            # A single malformed/corrupt series must not discard the entire study.
            # Record it and continue with the remaining acquisitions.
            audit_rows.append(
                {
                    UID_COLUMN: study_uid,
                    SERIES_UID_COLUMN: series_uid,
    ...
    # If every normal series was short or otherwise unusable, use a narrowly
    # scoped deterministic depth-repair path. This preserves all 4,405 existing
    # caches and only changes the exceptional studies that could not be cached.
    if not prepared_series:
        ...
        for _, row in ordered.iterrows():
            ...
            try:
                windows, starts, centers, audit = prepare_exceptional_series_window(
                    study_uid,
                    series_uid,
                    metadata_plane,
                )
                ...
                repair_errors.append(f"{series_uid}:{type(exc).__name__}:{exc}")

        if not prepared_series:
            raise RuntimeError(
                f"Study {study_uid} has no normal Ortho-eligible series and "
                f"exceptional repair also failed. Attempts={repair_errors[:10]}"
            )
    ...
    for series_index, item in enumerate(prepared_series):
        ...
        audit["Status"] = "OK_REPAIR" if item.get("repair", False) else "OK"
    ...
    payload = {
        ...
        "repair_version": (
            CACHE_REPAIR_VERSION
            if any(item.get("repair", False) for item in prepared_series)
            else None
        ),
    }
    ...
    for local_index, uid in enumerate(study_uids, start=1):
        ...
        if cache_file_is_usable(path, uid):
            ...
            try:
                ...
                log(
                    f"W5 CACHE FAILURE uid={uid} worker={worker_id} "
                    f"device={device}: {error_text}"
                )
...
def summarize_cache(train_df: pd.DataFrame, identity: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if len(audit_df) and "Status" in audit_df.columns:
        ...
        ok_audit = audit_df[
            audit_df["Status"].astype(str).isin(["OK", "OK_REPAIR"])
        ].copy()
    else:
        status_counts = {}
    ...
    summary = {
        ...
        "repaired_series": (
            int((audit_df["Status"].astype(str) == "OK_REPAIR").sum())
            if len(audit_df) and "Status" in audit_df.columns
            else 0
        ),
        "cache_repair_version": CACHE_REPAIR_VERSION,
...
def w50_status() -> Dict[str, Any]:
    ...
    payload = {
        ...
        "window_center_fractions": list(WINDOW_CENTER_FRACTIONS),
        "max_windows_per_series": MAX_WINDOWS_PER_SERIES,
        "exceptional_cache_repair_version": CACHE_REPAIR_VERSION,
        "exceptional_repair_policy": (
            "Only when a study has zero normal eligible series: decode all usable "
            "slices per series, depth-interpolate to 16, mark OK_REPAIR. Existing "
            "normal study caches remain untouched."
        ),
    }
