%%writefile /kaggle/working/w43_rsna_radimagenet_densenet121_2p5d_v7.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
RSNA Knee Abnormality Detection
W43 — RadImageNet DenseNet-121 2.5D Multi-Plane v7
==========================================================================
...
Kaggle T4 multi-GPU policy
--------------------------
W43 v7 intentionally does NOT use torch.nn.DataParallel.

v6 proved that the same FP32 DenseNet forward is numerically valid, but the
training path crashed inside a DataParallel replica with a cuDNN
"GET was unable to find an engine" error followed by a CUDA misaligned-address
failure. DataParallel also uses worker threads, while CUDA autocast state is
thread-local, making this mixed FP32-backbone / FP16-head policy unnecessarily
fragile.

For W43 v7:
    visible T4 GPUs       -> reported normally
    training GPU          -> cuda:0 only
    nn.DataParallel       -> disabled
    DenseNet backbone     -> FP32
    Transformer/head      -> FP16 on T4
    GradScaler            -> enabled

This is a deliberate stability choice for this experiment. No accelerator
environment variables are required.

Native BF16 detection
---------------------
PyTorch's torch.cuda.is_bf16_supported() may report True when BF16 is available
through emulation. W43 v7 does NOT use that result by itself.

v6 requires CUDA compute capability >= 8.0 for native BF16 selection.
Therefore:
    Tesla T4 / Turing / CC 7.5 -> FP16 autocast + GradScaler
    Ampere+ / CC >= 8.0       -> BF16 when natively supported

The RadImageNet DenseNet backbone remains FP32 in either case.

DenseNet / Kaggle-T4 precision policy
--------------------------------------
W43 v7 forces the RadImageNet DenseNet-121 backbone to FP32.
...
running variances (also reported publicly for this checkpoint). W43 v7 does
NOT train against those source-domain running statistics unchanged.

Before Stage-P pseudo training, W43 v7:
...
W43 v7 performs a real pixel-decode preflight before cache construction:
...
CLI
---
    python w43_rsna_radimagenet_densenet121_2p5d_v7.py status --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v7.py cache \
        --accelerator localGPU --cache-workers 8

    python w43_rsna_radimagenet_densenet121_2p5d_v7.py train_pseudo \
        --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v7.py train_cv \
        --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v7.py train_full \
        --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v7.py train_all \
        --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v7.py validate \
        --accelerator localGPU
...
SCRIPT_VERSION = "w43_radimagenet_densenet121_2p5d_v7"
DISPLAY_VERSION = "W43 | RadImageNet DenseNet-121 2.5D Multi-Plane v7"
OUTPUT_DIR_NAME = "rsna_w43_radimagenet_densenet121_2p5d_v7"
...
TRAINING_BACKWARD_PREFLIGHT_BATCHES = 1

# DataParallel is disabled for W43 because the mixed FP32 DenseNet / AMP head
# path crashed inside Kaggle T4 DataParallel replicas. One T4 is used reliably.
SAFE_SINGLE_GPU_TRAINING = True
...
def native_cuda_bf16_supported(device_index: int = 0) -> bool:
    """
    Return True only for native CUDA BF16 hardware.

    NVIDIA CUDA requires compute capability >= 8.0 for native BF16.
    PyTorch's is_bf16_supported() may include emulation, so we gate first by
    device capability and then ask PyTorch with emulation disabled when the
    installed PyTorch version supports that argument.
    """
    ...
    try:
        ...
        # Older PyTorch without the including_emulation keyword. Capability
        # >=8.0 is already enforced above, so this fallback remains safe.
...
def resolve_runtime(accelerator: str) -> Runtime:
    ...
    if value in {"local_gpu", "kaggle_t4"}:
        ...
        dtype = torch.bfloat16 if native_cuda_bf16_supported(0) else torch.float16
        return Runtime(
            requested,
            "cuda",
            device,
            count,
            dtype,
            False if SAFE_SINGLE_GPU_TRAINING else (count > 1),
        )
...
def create_new_training_model(
    ...
    if runtime.use_data_parallel:
        raise RuntimeError(
            "W43 v7 forbids nn.DataParallel. SAFE_SINGLE_GPU_TRAINING must remain enabled."
        )
...
def checkpoint_config(_paths=None) -> Dict[str, Any]:
    return {
        ...
        "native_bf16_only": True,
        "safe_single_gpu_training": SAFE_SINGLE_GPU_TRAINING,
        "training_backward_preflight_batches": TRAINING_BACKWARD_PREFLIGHT_BATCHES,
        "cache_version": CACHE_VERSION,
...
def global_study_batch(args, runtime: Runtime) -> int:
    ...
    if runtime.backend != "cuda":
        ...
        devices = runtime.visible_cuda_devices if runtime.use_data_parallel else 1
...
def trainable_gradient_summary(model: nn.Module) -> Dict[str, Any]:
    finite = True
    parameter_tensors = 0
    parameter_numel = 0
    grad_numel = 0
    grad_abs_max = 0.0
    nonfinite_examples: List[str] = []
    ...
    for name, parameter in core.named_parameters():
        ...
        if not torch.isfinite(grad).all():
            finite = False
            if len(nonfinite_examples) < 10:
                nonfinite_examples.append(name)

    return {
        "trainable_parameter_tensors": int(parameter_tensors),
        "trainable_parameter_numel": int(parameter_numel),
        "gradient_numel": int(grad_numel),
        "gradient_abs_max": float(grad_abs_max),
        "all_gradients_finite": bool(finite),
        "nonfinite_gradient_examples": nonfinite_examples,
    }


def training_backward_preflight(
    model: nn.Module,
    loader: DataLoader,
    runtime: Runtime,
) -> Dict[str, Any]:
    """
    Exercise the exact TRAINING execution path, including autograd and
    GradScaler, before epoch 1.

    Two stages are tested on one real cached-knee batch:
      frozen -> head-only backward
      full   -> DenseNet + head backward

    No optimizer step is performed, so model weights are unchanged.
    """
    ...
    for stage in ("frozen", "full"):
        ...
        if not torch.isfinite(logits).all():
            raise RuntimeError(
                f"Training backward preflight: non-finite logits in stage={stage}. "
        ...
        grad_summary = trainable_gradient_summary(model)
        if (
            not math.isfinite(float(loss.detach().cpu()))
            or grad_summary["gradient_numel"] <= 0
            or not grad_summary["all_gradients_finite"]
        ):
            raise RuntimeError(
                "W43 training/backward preflight FAILED.\n"
                + json.dumps(
                    {
                        "stage": stage,
                        "uids": batch_uids,
        ...
        optimizer.zero_grad(set_to_none=True)
        del optimizer, scaler, output, logits, loss
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    # Stage-P epoch 1 begins frozen; restore that exact state.
    set_backbone_train_stage(model, "frozen")
    set_backbone_batchnorm_eval(model)
    ...
    return {
        ...
        "visible_cuda_devices": int(runtime.visible_cuda_devices),
        "stages_checked": ["frozen", "full"],
        "records": records,
        "overall_pass": True,
    }
...
def run_train_pseudo(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    ...
    log("=" * 100)
    log(f"{DISPLAY_VERSION} | TRAINING + BACKWARD PREFLIGHT")
    log("=" * 100)
    backward_summary = training_backward_preflight(
        model,
        calibration_loader,
        runtime,
    )
    write_json(
        paths.result_root / "09c_training_backward_preflight.json",
        backward_summary,
    )
    for record in backward_summary["records"]:
        log(
            f"preflight stage={record['stage']:<6} "
            f"logit_abs_max={record['logits']['abs_max']:.6g} "
            f"loss={record['loss']:.6f} "
            f"grad_abs_max={record['gradients']['gradient_abs_max']:.6g}"
        )
    log("Training + backward preflight: PASS")
    ...
    log(f"Visible CUDA devices       : {runtime.visible_cuda_devices}")
    log(f"Training device            : {runtime.device}")
    log(f"DataParallel               : {runtime.use_data_parallel}")
    log(f"Multi-GPU policy           : safe_single_gpu")
    ...
    checkpoint_record = save_model_checkpoint(
        ...
        extra={
            ...
            "training_backward_preflight": backward_summary,
...
def run_status(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "runtime": {
            ...
            "training_cuda_devices": (1 if runtime.backend == "cuda" else 0),
            "multi_gpu_policy": (
                "safe_single_gpu" if runtime.backend == "cuda" else "not_applicable"
            ),
            "data_parallel_disabled_reason": (
                "Kaggle T4 DataParallel training crashed in the DenseNet cuDNN "
                "convolution path; W43 v7 uses cuda:0 for stability."
                if runtime.backend == "cuda"
                else None
            ),
            "accelerator_env_variables_required": False,
            "cuda_device_name": (
                torch.cuda.get_device_name(0) if runtime.backend == "cuda" else None
            ),
            ...
            "native_bf16_supported": (
                native_cuda_bf16_supported(0) if runtime.backend == "cuda" else False
            ),
        },
...
def run_validate(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    ...
    if pseudo_ckpt.exists():
        try:
            ...
            backward_info = pseudo_payload.get("extra", {}).get(
                "training_backward_preflight", {}
            )
            checks["pseudo_training_backward_preflight_pass"] = bool(
                backward_info.get("overall_pass") is True
                and backward_info.get("safe_single_gpu_training") is True
                and backward_info.get("data_parallel") is False
                and backward_info.get("stages_checked") == ["frozen", "full"]
            )
            del model
        except Exception:
        ...
        checks["pseudo_training_backward_preflight_pass"] = False
