...
W43 — RadImageNet DenseNet-121 2.5D Multi-Plane v3
==========================================================================
...
running variances (also reported publicly for this checkpoint). W43 v3 does
NOT train against those source-domain running statistics unchanged.

Before Stage-P pseudo training, W43 v3:
...
W43 v3 performs a real pixel-decode preflight before cache construction:
...
CLI
---
    python w43_rsna_radimagenet_densenet121_2p5d_v3.py status --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v3.py cache \
        --accelerator localGPU --cache-workers 8

    python w43_rsna_radimagenet_densenet121_2p5d_v3.py train_pseudo \
        --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v3.py train_cv \
        --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v3.py train_full \
        --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v3.py train_all \
        --accelerator localGPU

    python w43_rsna_radimagenet_densenet121_2p5d_v3.py validate \
        --accelerator localGPU
...
SCRIPT_VERSION = "w43_radimagenet_densenet121_2p5d_v3"
DISPLAY_VERSION = "W43 | RadImageNet DenseNet-121 2.5D Multi-Plane v3"
OUTPUT_DIR_NAME = "rsna_w43_radimagenet_densenet121_2p5d_v3"
...
BN_STRATEGY = "reset_recalibrate_on_w40_unlabeled_then_freeze_running_stats"
...
def load_train_tables(paths: ProjectPaths) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    for required in (
        paths.train_csv,
    ...
    bad_planes = set(series["Anatomical_Plane"].dropna().astype(str).unique()) - valid_planes
    if bad_planes:
...
def load_w40_teacher(
    ...
    for frame, name in ((probs, "probabilities"), (weights, "weights"), (masks, "masks")):
        ...
        if len(frame) != EXPECTED_UNLABELED:
            raise RuntimeError(f"W40 {name} rows={len(frame)}, expected={EXPECTED_UNLABELED}")
...
def inspect_dicom_decoder_support(
    ...
    failed_syntaxes = [
        value
        for value in decode_results.values()
        if value.get("decoded") is not True
    ]
...
def require_dicom_decoder_preflight(
    ...
    if not audit.get("overall_pass"):
        ...
        raise RuntimeError(
            ...
            "The RSNA dataset contains mixed compressed transfer syntaxes; "
            "pydicom alone may not decode them.\n"
            + (f"Failed syntaxes:\n{failed_text}\n" if failed_text else "")
            + f"Recommended decoder install:\n  {DICOM_DECODER_INSTALL_COMMAND}\n"
            "After installing the codecs, rerun `status` and then `cache`. "
            "Existing valid cache files are reusable; do NOT use --reset-cache."
        )
...
def build_study_2p5d_cache(
    ...
    for series_ordinal, (_, row) in enumerate(study_series.iterrows()):
        ...
        valid_iop = next((item["iop"] for item in records if item["iop"] is not None), None)
        ...
        if valid_iop is not None:
            ...
            if geometry_plane != metadata_plane and geometry_confidence >= GEOMETRY_PLANE_CONFIDENCE:
        ...
        for center in centers:
            ...
            try:
                for offset in STACK_OFFSETS:
                    ...
                    image, _ds, actual = _decode_record(records, target, orientation_spec)
                ...
                images.append(stack)
                plane_ids.append(int(PLANE_TO_INDEX.get(plane_used, PLANE_TO_INDEX[metadata_plane])))
                fluid_ids.append(fluid)
                fs_ids.append(fat_suppression)
                positions.append(float(normalized_slice_position(records, actual_center)))
...
def cache_file_is_usable(path: Path, uid: str) -> bool:
    ...
    try:
        with np.load(path, allow_pickle=False) as payload:
            ...
            n = images.shape[0]
            if images.ndim != 4 or images.shape[1:] != (3, CACHE_IMAGE_SIZE, CACHE_IMAGE_SIZE):
                return False
            if images.dtype != np.uint8 or n <= 0:
                return False
            for key in ("plane", "fluid", "fat_suppression", "slice_position", "series_ordinal"):
...
def run_cache(paths: ProjectPaths, args) -> Dict[str, Any]:
    if pydicom is None:
        raise RuntimeError("pydicom is required. Install pydicom before running cache mode.")
    ...
    grouped = {
        str(uid): group.copy()
        for uid, group in series.groupby(UID_COLUMN)
    }

    missing_series = [uid for uid in train[UID_COLUMN].astype(str) if uid not in grouped]
    if missing_series:
    ...
    todo = [
        uid
        for uid in train[UID_COLUMN].astype(str)
        if args.reset_cache or not cache_file_is_usable(study_cache_path(paths, uid), uid)
    ]
    ...
    if todo:
        with ThreadPoolExecutor(max_workers=max(1, int(args.cache_workers))) as executor:
...
class DenseNet121RadImageNetBackbone(nn.Module):
    ...
    def __init__(self):
        ...
        if len(encoder_layers) < 2:
            raise RuntimeError(
                "Unexpected torchvision DenseNet-121 child structure."
            )
        self.backbone = nn.Sequential(*encoder_layers[:-1])
...
def _extract_checkpoint_state(payload: Any) -> Dict[str, torch.Tensor]:
    ...
    raise RuntimeError(
        "Could not identify a PyTorch state_dict inside DenseNet121.pt."
    )
...
class RadImageNetDenseNet121StudyModel(nn.Module):
    ...
    def forward(
        ...
        flat_images = images.reshape(batch_size * max_tokens, 3, images.shape[-2], images.shape[-1])
        ...
        valid_features = self.backbone(valid_images)
        ...
        logits = (
            study_rep * self.classifier_weight.unsqueeze(0)
...
class StudyCacheDataset(Dataset):
    ...
    def _select_indices(self, uid: str, plane: np.ndarray, n: int) -> np.ndarray:
        ...
        if not self.training:
            return np.linspace(0, n - 1, min(self.max_tokens, n)).round().astype(np.int64)
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.training:
            ...
            if np.random.random() < 0.35:
                images = images + np.random.normal(0.0, 0.015, size=images.shape).astype(np.float32)
        ...
        return {
            ...
            "target_weight": torch.from_numpy(self.target_weights[uid].astype(np.float32)),
        }
...
def build_gold_target_maps(gold: pd.DataFrame) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray], Dict[str, np.ndarray]]:
    targets = {}
    masks = {}
    weights = {}
...
def build_pseudo_target_maps(
    ...
    for uid in p.index.astype(str):
        ...
        target_masks[uid] = np.asarray(
            [bool(coerce_bool_series(pd.Series([m.loc[uid, label]])).iloc[0]) for label in LABEL_COLUMNS],
            dtype=bool,
        )
...
def masked_soft_bce(
    ...
    effective = target_mask.float() * target_weight.float()
    denominator = effective.sum().clamp_min(1.0)
    return (loss * effective).sum() / denominator
...
def create_new_training_model(paths: ProjectPaths, runtime: Runtime) -> Tuple[nn.Module, Dict[str, Any]]:
    ...
    if runtime.use_data_parallel:
        model = nn.DataParallel(model, device_ids=list(range(runtime.visible_cuda_devices)))
...
def train_joint_epoch(
    ...
    for _step in range(JOINT_STEPS_PER_EPOCH):
        ...
        with autocast_context(runtime):
            ...
            loss = (
                JOINT_GOLD_AUTHORITY * gold_loss
                + JOINT_PSEUDO_AUTHORITY * pseudo_loss
...
def run_train_pseudo(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    ...
    pseudo_targets, pseudo_masks, pseudo_weights = build_pseudo_target_maps(probs, weights, masks)
    ...
    pd.DataFrame(history).to_csv(paths.result_root / "10_pseudo_history.csv", index=False)
    summary = {
        "script_version": SCRIPT_VERSION,
...
def run_train_cv(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    ...
    for fold in range(1, 6):
        ...
        train_uids = [uid for uid in gold[UID_COLUMN].astype(str) if fold_map[uid] != fold]
        val_uids = [uid for uid in gold[UID_COLUMN].astype(str) if fold_map[uid] == fold]
        ...
        history = []
        log("-" * 96)
        log(f"CV fold {fold} | seed={seed} | train={len(train_uids)} val={len(val_uids)}")
    ...
    log(f"Weak labels < .55    : {len(summary['weak_labels'])} -> {summary['weak_labels']}")
...
def run_train_full(paths: ProjectPaths, args, runtime: Runtime, force: bool = False) -> Dict[str, Any]:
    paths.ensure_output_dirs()
    train, _series, gold, unlabeled = load_train_tables(paths)
    probs, weights, masks, _w40_summary = load_w40_teacher(paths, unlabeled)
    ...
    log(f"Global study batch        : {batch_size}")
    log(f"Joint authority           : gold={JOINT_GOLD_AUTHORITY:g}, pseudo={JOINT_PSEUDO_AUTHORITY:g}")
    ...
    for seed in FULL_SEEDS:
        ...
        pseudo_loader = make_loader(pseudo_dataset, batch_size, True, args.loader_workers)
...
def run_status(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    ...
    if series is not None and dependency["pydicom"]:
        try:
            ...
            if not dicom_decoder_audit.get("overall_pass"):
                dicom_decoder_error = (
                    "One or more sampled DICOM transfer syntaxes cannot be pixel-decoded. "
                    f"Install codecs with: {DICOM_DECODER_INSTALL_COMMAND}"
                )
        except Exception as exc:
    ...
    if unlabeled is not None:
        try:
            ...
            teacher_info = {
                ...
                "changed_probabilities": w40_summary.get("controlled_changes", {}).get("probabilities_changed"),
                "weights_preserved": w40_summary.get("controlled_changes", {}).get("all_teacher_weights_preserved"),
                "masks_preserved": w40_summary.get("controlled_changes", {}).get("all_teacher_masks_preserved"),
            }
        except Exception as exc:
    ...
    payload = {
        ...
        "paths": {
            ...
            "radimagenet_weights": str(paths.radimagenet_weights) if paths.radimagenet_weights else None,
        },
        "data": {
            "error": data_error,
...
def run_validate(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    ...
    if paths.radimagenet_weights is not None:
        try:
            rad_audit = audit_radimagenet_weights(paths.radimagenet_weights)
            checks["radimagenet_checkpoint_exact_enough"] = bool(rad_audit["overall_pass"])
        except Exception:
        ...
        checks["radimagenet_checkpoint_exact_enough"] = pseudo_checkpoint_path(paths).exists()
    ...
    if pseudo_ckpt.exists():
        try:
            ...
            checks["pseudo_bn_recalibration_pass"] = bool(
                ...
                and int(bn_info.get("calibration_stacks", 0)) >= BN_CALIBRATION_MIN_STACKS
    ...
    if cv_summary_path.exists():
        cv_summary = json.loads(cv_summary_path.read_text(encoding="utf-8"))
        checks["cv_summary_version"] = cv_summary.get("script_version") == SCRIPT_VERSION
        checks["cv_verdict_valid"] = cv_summary.get("verdict") in {"GO_TO_FULL", "REVIEW", "STOP"}
    else:
        cv_summary = {}
    ...
    if full_summary_path.exists():
        ...
        checks["full_summary_version"] = full_summary.get("script_version") == SCRIPT_VERSION
        for seed in FULL_SEEDS:
            checkpoint = paths.checkpoint_root / "full" / f"seed_{seed}_final.pt"
            ...
            if checkpoint.exists():
                try:
                    model = create_empty_model(runtime)
                    payload = load_model_checkpoint(model, checkpoint, expected_scope="full_all58")
                    checks[f"full_seed_{seed}_strict_load"] = int(payload.get("seed", -1)) == seed
                    del model
                except Exception:
...
def run_train_all(paths: ProjectPaths, args, runtime: Runtime) -> Dict[str, Any]:
    if summarize_cache(paths, load_train_tables(paths)[0])["usable_studies"] != EXPECTED_TRAIN:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "mode",
        nargs="?",
        choices=["status", "cache", "train_pseudo", "train_cv", "train_full", "train_all", "validate"],
        default=None,
    )
    parser.add_argument(
        "--mode",
