...
#
# V2 IMPORTANT:
# - checkpoint/weight discovery NEVER recursively scans /kaggle/input
# - use RSNA_BUNDLE_ROOT=/kaggle/input/<your-bundle-dataset> for the
#   fastest and most deterministic hidden-test run.
...
WORK_ROOT = Path(
    os.environ.get(
        ...
        "/kaggle/working/rsna_w3_submission_final",
    )
)
...
# Optional attached Kaggle Dataset root, e.g.
# /kaggle/input/rsna-w3-submission-bundle
#
# Setting this is the fastest/most deterministic final-submission setup.
EXPLICIT_BUNDLE_ROOT = os.environ.get(
    "RSNA_BUNDLE_ROOT",
    "/kaggle/input/datasets/isayem/rsna-w3-submission-bundle",
).strip()
...
def _immediate_kaggle_input_roots() -> List[Path]:
    """
    Return only immediate Kaggle Dataset/Competition mount roots.

    CRITICAL:
    Never recursively scan /kaggle/input. The competition mount is
    hundreds of GB and recursive rglob discovery can waste minutes.
    """
...
def candidate_checkpoint_roots() -> List[Path]:
    ...
    # Explicit bundle root is preferred for a fresh scoring notebook.
    if EXPLICIT_BUNDLE_ROOT:

        bundle = Path(EXPLICIT_BUNDLE_ROOT)

        candidates.extend(
            [
                bundle / "w3_0_checkpoints",
                bundle / "w3_1_checkpoints",
                bundle / "rsna_w3_submission_bundle" / "w3_0_checkpoints",
                bundle / "rsna_w3_submission_bundle" / "w3_1_checkpoints",
            ]
        )
    ...
    candidates.extend(
        [
            ...
            Path("/kaggle/working/rsna_w3_submission_bundle/" "w3_0_checkpoints"),
            Path("/kaggle/working/rsna_w3_submission_bundle/" "w3_1_checkpoints"),
        ]
    )

    # Attached Kaggle Datasets: inspect only direct mount children and
    # at most one expected bundle subdirectory. No recursive DICOM scan.
    for root in _immediate_kaggle_input_roots():

        candidates.extend(
            [
                root / "w3_0_checkpoints",
                root / "w3_1_checkpoints",
                root / "rsna_w3_submission_bundle" / "w3_0_checkpoints",
                root / "rsna_w3_submission_bundle" / "w3_1_checkpoints",
            ]
        )
...
def discover_resnet18_state_dict_file() -> Optional[Path]:
    ...
    if EXPLICIT_BUNDLE_ROOT:
        ...
        candidates.extend(
            [
                bundle / RESNET18_BUNDLE_FILENAME,
                bundle / OFFICIAL_RESNET18_FILENAME,
                bundle / "rsna_w3_submission_bundle" / RESNET18_BUNDLE_FILENAME,
            ]
        )
    ...
    candidates.extend(
        [
            ...
            Path("/kaggle/working/rsna_w3_submission_bundle")
            / RESNET18_BUNDLE_FILENAME,
        ]
    )

    # Attached Kaggle Dataset roots only. Never recursively scan the
    # giant competition data mount.
    for root in _immediate_kaggle_input_roots():

        candidates.extend(
            [
                root / RESNET18_BUNDLE_FILENAME,
                root / OFFICIAL_RESNET18_FILENAME,
                root / "rsna_w3_submission_bundle" / RESNET18_BUNDLE_FILENAME,
            ]
        )
...
def run_inference() -> None:
    ...
    if EXPLICIT_BUNDLE_ROOT:
        print(f"Bundle root           : {EXPLICIT_BUNDLE_ROOT}")
