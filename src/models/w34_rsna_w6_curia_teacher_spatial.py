...
------------
    ...
    python rsna_w6_curia_teacher_spatial.py prepare_curia --accelerator localGPU
...
class TestFeatureStore(FeatureStore):
    ...
    def path_for_uid(self, uid: str) -> Path:
        ...
        # First prefer the exact requested test-cache variant.
        own = _test_cache_path(self.variant, uid)
        if own.exists():
            return own

        # W61 contains the original CLS features plus spatial patch-grid data,
        # so W60 can safely consume its CLS view when a standalone W60
...
def _cli() -> None:
    ...
    # log("STATUS")
    # log("=" * 96)
    # result = run_w6("status", "kaggle_t4")

    # log("\n" + "=" * 96)
    # log("CACHE_W61")
    # log("=" * 96)
    # result = run_w6("cache_w61", "kaggle_t4")
    # print(result)

    # log("\n" + "=" * 96)
    # log("TRAIN_W61_CV")
    # log("=" * 96)
    # result = run_w6("train_w61_cv", "kaggle_t4")

    log("\n" + "=" * 96)
    log("TRAIN_W61_FULL")
    log("=" * 96)
    result = run_w6("train_w61_full", "kaggle_t4")

    # global FINAL_VARIANTS
    # FINAL_VARIANTS = ["w60"]

    # global FINAL_ENSEMBLE_MODE
    # FINAL_ENSEMBLE_MODE = "rankmean"

    # print(f"$env:FINAL_VARIANTS_CLI='{FINAL_VARIANTS}'")

    # log("\n" + "=" * 96)
    # log("TRAIN_W61_FULL")
    # log("=" * 96)
    # result = run_w6("submit", "kaggle_t4")
    print(result)
