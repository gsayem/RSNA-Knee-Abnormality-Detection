...
EXPECTED_CURIA_MODEL_SHA256 = (
    "403a02e27531d2858ecd1e9b1ec2d5ea"
    "7bfa909f10ff0d9e8416090a6a8c96ef"
)
...
CACHE_VERSION = (
    "w4_0_curia2_cls_allseries_24slice_"
    "canonical_orientation_slow_processor_v1"
)
...
EXPECTED_FOLD_SHA256 = (
    "1d9959b027c055974325f4de59e26974"
    "b036ae8b2c1b63aa417d3eef7aaf9f4a"
)
...
def greedy_multilabel_folds(
    ...
    for i in range(n_samples):
        ...
        sample_rarity[i] = (
            float(np.sum(1.0 / label_frequency[positive]))
            if len(positive)
            else 0.0
        )
...
def load_training_tables() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, np.ndarray]:
    ...
    bad_planes = set(series_df["Anatomical_Plane"].dropna().astype(str).unique()) - valid_planes
    if bad_planes:
        raise RuntimeError(f"Unexpected anatomical planes: {sorted(bad_planes)}")

    gold_df = (
        train_df[
            train_df[LABEL_COLUMNS].notna().all(axis=1)
        ]
        .copy()
...
def validate_curia_identity(curia_root: Path) -> Tuple[str, Dict[str, Any]]:
    ...
    if (
        model_sha != EXPECTED_CURIA_MODEL_SHA256
        and not ALLOW_CURIA_SHA_MISMATCH
    ):
        raise RuntimeError(
...
def array_axis_vectors_from_iop(image_orientation_patient) -> Tuple[np.ndarray, np.ndarray]:
...
def orientation_transform_spec(
    ...
    identity_score = (
        abs(float(np.dot(current0, target0)))
        + abs(float(np.dot(current1, target1)))
    )

    transpose_score = (
        abs(float(np.dot(current1, target0)))
        + abs(float(np.dot(current0, target1)))
    )
...
def decode_dicom_pixel_array(ds, dicom_path: str) -> np.ndarray:
    ...
    try:
        ...
        try:
            is_compressed = bool(transfer_syntax.is_compressed) if transfer_syntax is not None else False
        except Exception:
            is_compressed = False
...
def cache_identity(model_sha: str) -> Dict[str, Any]:
    return {
        ...
        "orientation_targets": {key: list(value) for key, value in PLANE_TARGET_AXES.items()},
...
def benchmark_curia_batch_size(curia_root: Path) -> int:
    ...
    near_best = passed[
        passed["SlicesPerSecond"] >= 0.98 * best_rate
...
def encode_study(
        ...
        for target_index in selected_targets:
            try:
                ...
                if orientation_spec is None:
                    iop = getattr(ds, "ImageOrientationPatient", None)
                    if iop is not None:
                        geometry_plane, geometry_confidence = geometry_plane_from_iop(iop)
                        plane_match = geometry_plane == metadata_plane
                        orientation_plane = (
                            geometry_plane
                            if (
                        ...
                        orientation_spec = orientation_transform_spec(iop, orientation_plane)
    ...
    for series_index, item in enumerate(series_raw):
        ...
        plane_index = PLANE_TO_INDEX.get(item["plane_used"], PLANE_TO_INDEX[item["metadata_plane"]])
        ...
        span_scaled = (
            float(np.clip(span_mm / 300.0, 0.0, 2.0))
            if np.isfinite(span_mm)
            else 0.0
        )
...
def build_feature_cache(
    ...
    grouped_series = {
        str(uid): group.copy()
        for uid, group in series_df.groupby(UID_COLUMN)
    }
    ...
    need = [
        uid
        for uid in all_uids
        if uid not in set(already_cached)
    ]
...
def summarize_cache(
    ...
    if len(ok_audit):
        ...
        min_orientation = float(orientation_values.min()) if orientation_values.notna().any() else float("nan")
    ...
    summary = {
        ...
        "mean_encoded_series_per_study": (
            encoded_series / max(cached_n, 1)
        ),
        "mean_encoded_slices_per_study": (
            encoded_slices / max(cached_n, 1)
        ),
        "curia_model_sha256": model_sha,
...
def verify_w23_outer_folds(
    ...
    actual = actual[[UID_COLUMN, "OuterFold"]].sort_values(UID_COLUMN).reset_index(drop=True)
    ...
    ).sort_values(UID_COLUMN).reset_index(drop=True)
...
def load_fold_pseudo_targets(
    ...
    selection = selection_df[LABEL_COLUMNS].apply(pd.to_numeric, errors="coerce").fillna(0.0)
    ...
    high_mask_path = (
        ...
        / "folds"
        / f"fold_{fold}"
        / "high_selection_candidate_mask_wide.csv"
    )
...
class CuriaFeatureStore:
    def __init__(self, train_df: pd.DataFrame):
        ...
        for n, uid in enumerate(self.uids, start=1):
            ...
            total_feature_bytes += record["features"].numel() * record["features"].element_size()
        ...
        log(
            f"Feature payload size : {total_feature_bytes / (1024 ** 3):.3f} GB FP16"
        )
    ...
    def make_batch(
        ...
        for batch_index, record in enumerate(records):
            ...
            slice_position[batch_index, :n_series, :n_slices] = record["slice_position"].float()
...
class CuriaHierarchicalDiagnosisHead(nn.Module):
    def __init__(self):
        ...
        if HEAD_HIDDEN_DIM % HEAD_NUM_HEADS != 0:
            raise ValueError(
                "HEAD_HIDDEN_DIM must be divisible by HEAD_NUM_HEADS."
            )
    ...
    def forward(
        ...
        pooled_series = self.series_norm(
            pooled_series + metadata_hidden.unsqueeze(1)
        )
        ...
        logits = (
            ...
            * self.classifier_weight.unsqueeze(0)
        ).sum(dim=-1) + self.classifier_bias.unsqueeze(0)
...
EXPECTED_W4_CONFIG_HASH = (
    "c0fba803bef4b924d76476e51cbdb7a36"
    "bffaef8603f8f8e3dcdc980c9f9b9e8"
)
...
EXPECTED_FOLD_GATE = {
    1: {
        ...
        "Effusion", "Baker's", "Contusion", "Fracture",
    },
    2: {
        "ACL", "MCL", "Medial Meniscus", "Lateral Meniscus",
        "Effusion", "Baker's", "Fracture",
    },
    3: {
        "ACL", "MCL", "Medial Meniscus", "Lateral Meniscus",
        "Effusion", "Baker's", "Contusion", "Fracture",
...
def looks_like_w4_artifact_root(root: Path) -> bool:
    return all(
        path.exists()
        for path in _all_expected_checkpoint_paths(root)
    )
...
def _validate_checkpoint_payload(
    ...
    if missing:
        raise RuntimeError(
            f"{path} missing checkpoint fields: {sorted(missing)}"
        )
    ...
    if int(payload["fold"]) != int(fold):
        raise RuntimeError(
            f"{path}: fold mismatch {payload['fold']} != {fold}"
        )
...
def derive_gate_from_training_results(
    ...
    for fold in range(1, NUM_FOLDS + 1):
        path = (
            ...
            / "results"
            / "fold_safe_weak"
            / f"fold_{fold}_pseudo_audit.csv"
        )
    ...
    payload = {
        "rule":
            "Use fold_safe_weak W4 head for a fold/label iff "
        ...
        "use_weak_labels_by_fold": {
            str(fold): [
                label
                for label in LABEL_COLUMNS
                if label in EXPECTED_FOLD_GATE[fold]
            ]
            for fold in range(1, NUM_FOLDS + 1)
        },

        "selected_cell_counts_by_fold":
            cell_counts,
    }
...
def reconstruct_gated_oof(
    ...
    for _, row in (
        ...
        if uid not in weak_indexed.index:
            raise RuntimeError(
                f"Weak OOF file missing UID {uid}"
            )
    ...
    expected = {
        "macro_AUROC":
            EXPECTED_GATED_OOF_AUROC,

        "macro_AP":
            EXPECTED_GATED_OOF_AP,

        "macro_F1":
            EXPECTED_GATED_OOF_F1,
    }
...
def load_gate_matrix_from_artifact_root(
    ...
    if gate_path.exists():
        ...
        if labels != LABEL_COLUMNS:
            raise RuntimeError(
                "Bundle fold gate label order mismatch."
            )
        ...
        matrix, _ = (
            derive_gate_from_training_results(
                root
            )
        )
    ...
    if matrix.shape != (
        ...
        raise RuntimeError(
            f"Unexpected fold-gate matrix shape {matrix.shape}."
        )
    ...
    if not np.array_equal(
        ...
        raise RuntimeError(
            "Fold-gate matrix differs from the validated W2.3 gate."
        )
...
def export_submission_bundle() -> Path:
    source_root = (
        discover_w4_artifact_root(
            EXPLICIT_TRAIN_ARTIFACT_ROOT
        )
    )
    ...
    log("=" * 88)
    log(
        f"Source W4 root        : {source_root}"
    )

    checkpoint_hashes = (
        validate_all_w4_checkpoints(
            source_root
        )
    )
    ...
    if abs(
        gold_auc
        - EXPECTED_GOLD_OOF_AUROC
    ) > 1e-6:
        raise RuntimeError(
            f"Unexpected gold OOF AUROC: {gold_auc}"
        )

    if abs(
        weak_auc
        - EXPECTED_WEAK_OOF_AUROC
    ) > 1e-6:
        raise RuntimeError(
            f"Unexpected weak OOF AUROC: {weak_auc}"
        )

    gate_matrix, gate_payload = (
        derive_gate_from_training_results(
            source_root
        )
    )

    gated_summary, gated_oof = (
        reconstruct_gated_oof(
            source_root,
            gate_matrix,
        )
    )
    ...
    (
        BUNDLE_EXPORT_ROOT
        / "CURIA_EXTERNAL_MODEL_NOTICE.txt"
    ).write_text(
        notice,
        encoding="utf-8",
    )
    ...
    manifest = {
        "bundle_version":
            "w4_0_curia2_final_submission_bundle_v1",
        ...
        "verification": {
            "gold_oof_macro_AUROC":
                gold_auc,

            "weak_oof_macro_AUROC":
                weak_auc,

            "fold_gated_oof_macro_AUROC":
                gated_summary[
                    "macro_AUROC"
                ],

            "fold_gated_oof_macro_AP":
                gated_summary[
                    "macro_AP"
                ],

            "fold_gated_oof_macro_F1":
                gated_summary[
                    "macro_F1"
                ],
        },
    ...
    if copied_hashes != checkpoint_hashes:
        raise RuntimeError(
            "Copied W4 checkpoint hashes do not match source hashes."
        )
    ...
    zip_path = Path(
        shutil.make_archive(
            ...
            "zip",
            root_dir=
                BUNDLE_EXPORT_ROOT.parent,
            base_dir=
                BUNDLE_EXPORT_ROOT.name,
        )
    )
    ...
    log("")
    log(
        f"Bundle created        : {BUNDLE_EXPORT_ROOT}"
    )

    log(
        f"Bundle zip            : {zip_path}"
    )
    ...
    log(
        f"Verified gated OOF    : "
        f"{gated_summary['macro_AUROC']:.6f}"
    )

    log("")
    log(
        "Final scoring notebook must attach TWO datasets:"
    )

    log(
        "  1. the original Curia-2 model dataset"
    )

    log(
        "  2. this W4 submission bundle dataset"
    )
...
TEST_CACHE_FAILURES_PATH = (
    SUBMISSION_RESULT_ROOT
    / "test_feature_cache_failures.csv"
)

TEST_CACHE_AUDIT_PATH = (
    SUBMISSION_RESULT_ROOT
    / "test_series_cache_audit.csv"
)
...
def load_test_tables() -> Tuple[
    ...
    if UID_COLUMN not in test_df.columns:
        raise RuntimeError(
            "test.csv is missing StudyInstanceUID."
        )

    if UID_COLUMN not in series_df.columns:
        raise RuntimeError(
            "test_series.csv is missing StudyInstanceUID."
        )

    if UID_COLUMN not in sample_df.columns:
        raise RuntimeError(
            "sample_submission.csv is missing StudyInstanceUID."
        )
    ...
    if missing_series:
        raise RuntimeError(
            "test_series.csv missing required columns: "
    ...
    if test_df[
        ...
        raise RuntimeError(
            "test.csv contains duplicate StudyInstanceUID values."
        )
    ...
    if set(
        ...
        raise RuntimeError(
            "test.csv and sample_submission.csv UID sets differ."
        )
...
def test_cache_identity(
    ...
    return {
        ...
        "processor":
            "official CuriaImageProcessor, use_fast=False",
...
def prepare_test_cache_identity(
    ...
    if RESET_TEST_CACHE:
        log(
            "W40_RESET_TEST_CACHE=1 -> deleting test feature cache."
        )

        for path in (
            TEST_CACHE_STUDY_ROOT.glob(
                "*.pt"
            )
        ):
            path.unlink()
    ...
    if TEST_CACHE_META_PATH.exists():
        actual = json.loads(
            TEST_CACHE_META_PATH.read_text(
                encoding="utf-8"
            )
        )
...
def _test_cache_worker(
    ...
    processor, model = (
        load_curia_processor_and_model(
            curia_root,
            device,
        )
    )
        ...
        if test_cache_file_is_usable(
            ...
            try:
                ...
                if (
                    ...
                    raise RuntimeError(
                        "No test_series.csv rows for study."
                    )
...
def summarize_test_cache(
    ...
    (
        SUBMISSION_RESULT_ROOT
        / "test_feature_cache_summary.json"
    ).write_text(
        json.dumps(
            summary,
            indent=2,
...
def build_test_feature_cache(
    ...
    curia_root = (
        discover_curia2_root()
    )

    model_sha, _ = (
        validate_curia_identity(
            curia_root
        )
    )

    prepare_test_cache_identity(
        model_sha
    )
    ...
    log("=" * 88)
    log(
        f"Curia root            : {curia_root}"
    )
    log(
        f"Curia SHA256          : {model_sha}"
    )
    log(
        f"CUDA GPU count        : {GPU_COUNT}"
    )
    log(
        f"Cache GPU IDs         : {CACHE_GPU_IDS}"
    )
    log(
        f"Curia FP16 batch      : {SUBMISSION_CURIA_BATCH}"
    )
    log(
        f"Test studies          : {len(study_uids)}"
    )
    log(
        f"Already cached        : {len(already_cached)}"
    )
    log(
        f"Need encoding         : {len(need)}"
    )
    log(
        f"Max slices / series   : {MAX_SLICES_PER_SERIES}"
    )
    ...
    try:
        with ThreadPoolExecutor(
            ...
            for worker_id, device_id in enumerate(
                worker_gpu_ids
            ):
                futures.append(
                    executor.submit(
    ...
    if not bool(
        ...
        raise RuntimeError(
            "Hidden-test Curia cache is incomplete."
        )
...
def load_w4_head(
    ...
    model = (
        CuriaHierarchicalDiagnosisHead()
        .to(
            device
        )
    )
...
def predict_all_w4_heads(
    ...
    if device.type != "cuda":
        raise RuntimeError(
            "W4 head inference expects CUDA."
        )
    ...
    for fold in range(
        ...
        log(
            f"Fold {fold}/{NUM_FOLDS} fold_safe_weak..."
        )
...
def validate_submission_dataframe(
    ...
    if submission[
        UID_COLUMN
    ].astype(str).tolist() != sample_df[
        UID_COLUMN
    ].astype(str).tolist():
        raise RuntimeError(
            f"{name}: StudyInstanceUID order mismatch."
        )
    ...
    if submission[
        ...
        raise RuntimeError(
            f"{name}: duplicate StudyInstanceUID."
        )
...
def save_prediction_outputs(
    ...
    primary_copy_path = (
        SUBMISSION_WORK_ROOT
        / "submission_w4_curia_fold_gated_primary.csv"
    )

    weak_path = (
        SUBMISSION_WORK_ROOT
        / "submission_w4_curia_weak_all.csv"
    )

    gold_path = (
        SUBMISSION_WORK_ROOT
        / "submission_w4_curia_gold_all.csv"
    )
...
def verify_bundle_manifest_if_present(
    ...
    if manifest.get(
        ...
        raise RuntimeError(
            "Bundle manifest W4 config hash mismatch."
        )
    ...
    for key, value in expected.items():
        ...
        if actual is None:
            raise RuntimeError(
                f"Bundle manifest missing verification {key}."
            )
        ...
        if abs(
            ...
            raise RuntimeError(
                f"Bundle verification mismatch for {key}: "
                f"{actual} vs {value}"
            )
    ...
    if expected_hashes:
        ...
        if actual_hashes != expected_hashes:
            raise RuntimeError(
                "Bundle checkpoint SHA256 verification failed."
            )
...
def run_submission_inference() -> Dict[str, Any]:
    ...
    log(
        f"Encoder GPUs          : {CACHE_GPU_IDS}"
    )
    log(
        f"Competition data root : {DATA_ROOT}"
    )
    log(
        f"Output                : {SUBMISSION_WORK_ROOT}"
    )
    log(
        "Primary               : W4 fold-gated Curia ensemble"
    )
    log(
        "Secondary             : W4 fold_safe_weak all-label ensemble"
    )

    artifact_root = (
        discover_w4_artifact_root(
            EXPLICIT_SUBMISSION_BUNDLE_ROOT
        )
    )

    log(
        f"W4 artifact root      : {artifact_root}"
    )
    ...
    log(
        f"Verified checkpoints  : {len(checkpoint_hashes)}"
    )

    manifest = (
        verify_bundle_manifest_if_present(
            artifact_root
        )
    )

    gate_matrix = (
        load_gate_matrix_from_artifact_root(
            artifact_root
        )
    )
    ...
    log(
        f"Curia-2 root          : {curia_root}"
    )
    log(
        f"Curia SHA256          : {curia_sha}"
    )

    test_df, series_df, sample_df = (
        load_test_tables()
    )

    log(
        f"Test studies          : {len(test_df)}"
    )
    log(
        f"Test series           : {len(series_df)}"
    )

    cache_summary = (
        build_test_feature_cache(
            test_df,
            series_df,
        )
    )
    ...
    final_manifest = {
        "submission_version":
            "w4_0_curia2_final_submission_v1",
    ...
    log(
        f"Total runtime          : "
        f"{elapsed_string(time.time() - started)}"
    )
    log("")
    log(
    ...
    if mode == "status":
        ...
        try:
            artifact_root = (
                discover_w4_artifact_root(
                    EXPLICIT_SUBMISSION_BUNDLE_ROOT
                    or EXPLICIT_TRAIN_ARTIFACT_ROOT
                )
            )
        except Exception as exc:
            artifact_error = repr(
        ...
        payload = {
            ...
            "test_series_csv_exists":
                TEST_SERIES_CSV.exists(),

            "test_series_root_exists":
                TEST_SERIES_ROOT.exists(),

            "sample_submission_exists":
                SAMPLE_SUBMISSION_CSV.exists(),
...
def parse_submission_args():
    parser = argparse.ArgumentParser(
        description=(
            "RSNA W4.0 Curia-2 final inference / bundle export"
        )
    )
...
def _w42_looks_like_w41_root(root: Path) -> bool:
    ...
    return all(
        _w42_checkpoint_path(root, seed).is_file()
...
def _w42_discover_w41_root(
    ...
    if not _w42_looks_like_w41_root(root):
        expected = [
            str(_w42_checkpoint_path(root, seed))
            for seed in W42_EXPECTED_W41_SEEDS
        ]
...
def _w42_verify_checkpoint_payload(
    ...
    if list(changes or []) != ["Contusion", "Effusion"]:
        raise RuntimeError(
            f"Seed {seed} unexpected teacher_changes_only={changes!r}"
        )
...
def _w42_verify_w41_root(root: Path, verify_sha: bool = True) -> Dict[str, Any]:
    ...
    return {
        ...
        "w41_validation_overall_pass": validation.get("overall_pass") if validation else None,
    }
...
def _w42_predict_all_seeds(
    ...
    shards = [
        W42_EXPECTED_W41_SEEDS[i::used_gpu_count]
        for i in range(used_gpu_count)
    ]
    ...
    with ThreadPoolExecutor(max_workers=used_gpu_count) as executor:
        ...
        for future in as_completed(futures):
            ...
            if overlap:
                raise RuntimeError(f"Duplicate seed inference result: {sorted(overlap)}")
...
def _w42_submission_dataframe(
    ...
    frame = (
        frame.set_index(UID_COLUMN)
        .loc[sample_df[UID_COLUMN].astype(str)]
        .reset_index()
    )
...
def _w42_validate_submission_frame(
    ...
    if frame[UID_COLUMN].astype(str).tolist() != sample_df[UID_COLUMN].astype(str).tolist():
        raise RuntimeError(f"{name}: submission UID order differs from sample_submission.csv")
...
def _w42_infer(args) -> Dict[str, Any]:
    ...
    log(
        distribution[
            ["Label", "RankMean", "RankStd", "RawMean"]
...
def _w42_validate(args, configure: bool = True) -> Dict[str, Any]:
    ...
    if manifest_path.is_file():
        try:
            ...
            checks["submission_sha256_match"] = (
                canonical_submission.is_file()
                and manifest.get("submission_sha256")
                == sha256_file(canonical_submission)
            )
        except Exception:
    ...
    if cache_summary_path.is_file():
        try:
            ...
            if sample_df is not None:
                checks["test_cache_study_count"] = (
                    int(cache_summary.get("test_studies", -1)) == len(sample_df)
                )
            else:
