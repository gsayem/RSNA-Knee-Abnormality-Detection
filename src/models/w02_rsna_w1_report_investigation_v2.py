...
## Local file
# Define the project root based on the script location (\src\models\my_model.py)
...
WORK_ROOT = os.path.join(PROJECT_ROOT, "output", "rsna_w1")
...
# 2. Check if required input files exist
if not os.path.isfile(TRAIN_CSV):
    raise FileNotFoundError(f"Required file does not exist: {TRAIN_CSV}")

if not os.path.isfile(TRAIN_SERIES_CSV):
    raise FileNotFoundError(f"Required file does not exist: {TRAIN_SERIES_CSV}")

# 3. Optional: Verify output/work directories
if not os.path.isdir(WORK_ROOT):
    print(f"Notice: WORK_ROOT directory will be used at: {WORK_ROOT}")
## End : Local File
...
def safe_json_value(value: Any) -> Any:
    ...
    # 3. Handle NumPy arrays or iterables (lists, tuples, arrays) before pd.isna
    ...
    # Check if it's a scalar or if pd.isna can be evaluated safely
    try:
...
# ============================================================
# 6A. HARD VALIDATION: ALL 58 GOLD STUDIES MUST HAVE REPORTS
# ============================================================
#
# W1 uses the gold cohort to connect report language/terms to
# the 12 ground-truth labels. Therefore every gold study must
# have non-empty report text. Fail loudly if this assumption
# is ever violated.

gold_source_check_df = train_df[train_df["LabelStatus"] == "gold"].copy()


if len(gold_source_check_df) != EXPECTED_GOLD_STUDIES:

    raise RuntimeError(
        "Expected exactly "
        f"{EXPECTED_GOLD_STUDIES} gold studies, "
        f"but found {len(gold_source_check_df)}."
    )


gold_report_missing_mask = gold_source_check_df[
    REPORT_COLUMN
].isna() | gold_source_check_df[REPORT_COLUMN].astype(str).str.strip().eq("")


gold_reports_missing = int(gold_report_missing_mask.sum())


if gold_reports_missing > 0:

    missing_gold_uids = (
        gold_source_check_df.loc[
            gold_report_missing_mask,
            UID_COLUMN,
        ]
        .astype(str)
        .tolist()
    ...
    raise RuntimeError(
        "All 58 gold-labeled studies must have "
        "non-empty Report text for W1. "
        f"Missing reports: {gold_reports_missing}. "
        f"StudyInstanceUIDs: {missing_gold_uids}"
    )


print(
    "Gold report validation: "
    f"{EXPECTED_GOLD_STUDIES}/{EXPECTED_GOLD_STUDIES} "
    "gold studies have non-empty Report text "
    "and all 12 labels."
)
...
inventory_path = os.path.join(RESULT_ROOT, "01_report_inventory.csv")
...
# )
save_json(
    os.path.join(RESULT_ROOT, "02_report_summary.json"),
    summary,
)
...
script_distribution_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "03_script_distribution.csv"),
    index=False,
)
...
language_distribution_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "04_language_distribution.csv"),
    index=False,
)
...
language_status_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "05_language_by_label_status.csv"),
    index=False,
)
...
script_status_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "06_script_by_label_status.csv"),
    index=False,
)
...
length_stats_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "07_report_length_stats.csv"),
    index=False,
)
...
quality_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "08_report_quality_flags.csv"),
    index=False,
)
...
duplicate_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "09_duplicate_report_groups.csv"),
    index=False,
)
...
near_duplicate_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "10_near_duplicate_reports.csv"),
    index=False,
)
...
examples_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "11_language_report_examples.csv"),
    index=False,
)
...
gold_report_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "12_gold_reports_with_labels.csv"),
    index=False,
)
...
term_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "13_gold_label_term_associations.csv"),
    index=False,
)
...
concordance_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "14_gold_term_sentence_concordance.csv"),
    index=False,
)
...
corpus_terms_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "15_frequent_terms_by_language.csv"),
    index=False,
)
...
cue_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "16_negation_uncertainty_cue_survey.csv"),
    index=False,
)
...
gold_label_summary_df.to_csv(
    ...
    os.path.join(RESULT_ROOT, "17_gold_label_report_summary.csv"),
    index=False,
)
...
markdown_path = os.path.join(RESULT_ROOT, "W1_REPORT.md")
...
config = {
    ...
    "gold_reports_verified_non_empty": True,
    "gold_reports_missing": gold_reports_missing,
...
save_json(
    ...
    os.path.join(RESULT_ROOT, "w1_config.json"),
    config,
)
