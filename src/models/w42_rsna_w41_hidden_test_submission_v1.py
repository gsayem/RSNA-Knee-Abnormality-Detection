...
import zipfile as _w42_zipfile

W42_SCRIPT_VERSION = "w42_w41_hidden_test_submission_v1"
W42_EXPERIMENT = "W42 | W41 exact W6.0 Curia CLS hidden-test submission"
...
def _w42_choose_output_root(explicit: Optional[str]) -> Path:
    if explicit:
        return Path(explicit).expanduser().resolve()
    if _w42_is_kaggle():
        return Path("/kaggle/working/rsna_w42_w41_submission_v1")
    return (
        _w42_project_root() / "output" / "results" / "rsna_w42_w41_submission_v1"
    ).resolve()
...
def _w42_candidate_w41_roots(explicit: Optional[str]) -> List[Path]:
    roots: List[Path] = []
    ...
    roots.extend(
        [
            project / "output" / "results" / "rsna_w41_w60_curia_w40_teacher_v1",
            Path("/kaggle/working/rsna_w41_w60_curia_w40_teacher_v1"),
        ]
    )

    if Path("/kaggle/input").exists():
        # Shallow deterministic directory search first.
        for dataset_root in Path("/kaggle/input").iterdir():
            if not dataset_root.is_dir():
                continue
            roots.extend(
                [
                    dataset_root,
                    dataset_root / "rsna_w41_w60_curia_w40_teacher_v1",
                ]
            )
...
def _w42_extract_w41_zip(zip_path: Path, output_root: Path) -> Path:
    destination = output_root / "w41_inference_bundle_extracted"
    checkpoint_dir = destination / "checkpoints" / "full"
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    (destination / "results" / "full").mkdir(parents=True, exist_ok=True)
    (destination / "results").mkdir(parents=True, exist_ok=True)

    wanted_suffixes = {
        **{
            f"checkpoints/full/seed_{seed}_epoch_24.pt": checkpoint_dir
            / f"seed_{seed}_epoch_24.pt"
            for seed in W42_EXPECTED_W41_SEEDS
        },
        "results/full/fullfit_summary.json": destination
        / "results"
        / "full"
        / "fullfit_summary.json",
        "results/validation_summary.json": destination
        / "results"
    ...
    with _w42_zipfile.ZipFile(zip_path, "r") as archive:
        for member in archive.namelist():
            normalized = member.replace("\\", "/").rstrip("/")
            for suffix, target in wanted_suffixes.items():
    ...
    required_checkpoint_suffixes = {
        f"checkpoints/full/seed_{seed}_epoch_24.pt" for seed in W42_EXPECTED_W41_SEEDS
    }
    if not required_checkpoint_suffixes.issubset(found):
        raise RuntimeError(
            f"W41 zip {zip_path} does not contain all three full-fit checkpoints."
        )

    return destination.resolve()


def _w42_find_w41_zip() -> Optional[Path]:
    candidates = []

    project = _w42_project_root()
    candidates.extend(
        [
            project / "rsna_w41_w60_curia_w40_teacher_v1.zip",
            project / "output" / "results" / "rsna_w41_w60_curia_w40_teacher_v1.zip",
        ]
    )
    ...
    if Path("/kaggle/input").exists():
        for dataset_root in Path("/kaggle/input").iterdir():
            if dataset_root.is_dir():
                candidates.extend(dataset_root.glob("*w41*.zip"))
                candidates.extend(dataset_root.glob("*W41*.zip"))
...
def _w42_discover_w41_root(explicit: Optional[str], output_root: Path) -> Path:
    if explicit:
        explicit_path = Path(explicit).expanduser().resolve()
        if explicit_path.is_file() and explicit_path.suffix.lower() == ".zip":
            return _w42_extract_w41_zip(explicit_path, output_root)

    for root in _w42_candidate_w41_roots(explicit):
        if _w42_looks_like_w41_root(root):
            return root.resolve()

        # Dataset may contain one additional top-level directory.
        if root.is_dir():
            try:
                for child in root.iterdir():
                    if child.is_dir() and _w42_looks_like_w41_root(child):
                        return child.resolve()
            except Exception:
                pass

    # Last directory-based recovery: locate seed 6001 and derive root.
    if Path("/kaggle/input").exists():
        for dataset_root in Path("/kaggle/input").iterdir():
            if not dataset_root.is_dir():
                continue
            try:
                for path in dataset_root.rglob("seed_6001_epoch_24.pt"):
                    parts = path.parts
                    if len(parts) >= 3 and path.parent.name == "full":
                        candidate = path.parent.parent.parent
                        if _w42_looks_like_w41_root(candidate):
                            return candidate.resolve()
            except Exception:
                pass

    zip_path = _w42_find_w41_zip()
    if zip_path is not None:
        return _w42_extract_w41_zip(zip_path, output_root)

    raise FileNotFoundError(
        "W41 inference artifacts not found. Attach/create a Kaggle Dataset containing "
        "checkpoints/full/seed_6001_epoch_24.pt, seed_6002_epoch_24.pt, "
        "seed_6003_epoch_24.pt, or pass --w41-root explicitly."
    )
