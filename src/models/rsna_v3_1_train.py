# ============================================================
# RSNA KNEE ABNORMALITY DETECTION - V3.1
#
...
#   -> normalized physical slice-position embedding
#   -> slice attention
#   -> series embedding
#   -> metadata fusion
...
#
# Phase: V3.1 stabilized fixed-epoch training protocol
#   - Single visual backbone
#   - ResNet18 layer4 fine-tuned exactly as V2
#   - Adds normalized physical slice positional encoding
#   - Fixed 12 epochs; no early stopping
#   - Deterministic cosine LR schedule
#   - Saves complete OOF predictions at every epoch
...
WORK_ROOT = Path("/kaggle/working/rsna_v3_1")

# V3 cache is separate from V1/V2 because each cached series
# additionally stores the sampled-slice positional tensor.
...
# V3.1: fixed training budget. Validation does not stop training.
...
# V3 controlled change:
# map one normalized slice position p in [0,1] into the
# same embedding space as the visual slice representation.
SLICE_POSITION_INPUT_DIM = 1
SLICE_POSITION_HIDDEN_DIM = 32
...
def normalized_sampled_slice_positions(
    headers: List[Dict[str, Any]],
    selected_indices: np.ndarray,
    all_spatial: bool,
) -> Tuple[torch.Tensor, str]:
    """
    Calculate one normalized position for each sampled slice.

    Preferred:
        DICOM physical coordinate from ImagePositionPatient
        projected onto the series slice normal.
    ...
    Returns:
        positions: [K] float32 in [0, 1]
        source: "dicom_physical" or "normalized_index"
    """

    n = len(headers)

    if n == 0:
        raise ValueError("Cannot calculate slice positions for an empty series.")

    if all_spatial:

        full_positions = np.asarray(
            [item["position"] for item in headers],
            dtype=np.float64,
        )

        if np.isfinite(full_positions).all():

            z_min = float(np.min(full_positions))

            z_max = float(np.max(full_positions))

            span = z_max - z_min

            if span > 1e-8:

                sampled_positions = full_positions[selected_indices]

                normalized = (sampled_positions - z_min) / span
    ...
    if n <= 1:
        ...
        normalized = selected_indices.astype(np.float32) / float(n - 1)
...
def load_sampled_series(
    ...
    Returns:
        ...
        slice_positions:
            [K] normalized sampled-slice positions in [0,1].
    """
    ...
    if USE_CACHE and cache_path.exists():
        ...
        if "slice_positions" in cached:

            return (
                cached["images"],
                cached["geometry"],
                cached["slice_positions"].float(),
            )
    ...
    # --------------------------------------------------------
    # V3: normalized sampled-slice positions
    # --------------------------------------------------------

    (
        slice_positions,
        slice_position_source,
    ) = normalized_sampled_slice_positions(
        headers,
        selected_indices,
        all_spatial,
    )
    ...
    geometry = {
        ...
        "slice_count": float(n),
        "slice_position_source": slice_position_source,
    }
        ...
        torch.save(
            {
                "images": images.half(),
                "geometry": geometry,
                "slice_positions": slice_positions.half(),
            },
            cache_path,
        )
...
class KneeStudyDataset(Dataset):
    ...
    def __getitem__(
        ...
        for _, series_row in series_rows.iterrows():
            ...
            series_slice_positions.append(slice_positions.float())
...
class V3HierarchicalModel(nn.Module):
    def __init__(
        self,
        metadata_dim: int,
        num_labels: int,
        ...
        # V3 position encoder.
        # p in [0,1] -> 32-D hidden -> 256-D positional embedding.
        self.slice_position_encoder = nn.Sequential(
            nn.Linear(
                SLICE_POSITION_INPUT_DIM,
                SLICE_POSITION_HIDDEN_DIM,
            ),
            nn.GELU(),
            nn.Linear(
                SLICE_POSITION_HIDDEN_DIM,
    ...
    def forward(
        ...
        # ----------------------------------------------------
        # V3 controlled change:
        # add spatial position before slice attention.
        # --------------------------------------------------------

        position_features = self.slice_position_encoder(
            slice_positions.reshape(
                B * S,
                K,
                1,
            ).float()
        ...
        slice_features = slice_features.float() + position_features.float()
...
def run_epoch(
    ...
    for batch in loader:
        ...
        slice_positions = batch["slice_positions"].to(
            DEVICE,
            non_blocking=True,
        )
...
# Stores one validation prediction per study for every epoch.
# Across five folds this becomes a complete 58-study OOF set
# for each epoch 1..12.
epoch_oof_records = []
...
for fold in range(NUM_FOLDS):
    ...
    # V3.1: validation-independent learning-rate schedule.
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=NUM_EPOCHS,
        eta_min=(BACKBONE_LEARNING_RATE * COSINE_ETA_MIN_FACTOR),
    )

    print("\nV3 fine-tuning + slice-position setup:")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # Reporting-only. This score does not alter training.
        ...
        #
        # They are not used for early stopping or checkpoint
        # selection. They are collected only to build a full
        # OOF learning curve after all folds finish.
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Fixed final-epoch checkpoint only.
        # ----------------------------------------------------
            ...
            torch.save(
                {
                    ...
                    "config": {
                        ...
                        "slice_position_encoding": "normalized_dicom_physical_with_index_fallback",
                        "slice_position_hidden_dim": SLICE_POSITION_HIDDEN_DIM,
...
print("\n")
print("=" * 90)
print("FINAL V3.1 FIXED-EPOCH OOF RESULTS")
print("=" * 90)
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    if len(epoch_df) != len(gold_df):
        ...
        raise RuntimeError(
            ...
            f"{len(gold_df)} complete OOF rows, "
    ...
    if epoch_df["StudyInstanceUID"].nunique() != len(gold_df):
        ...
        raise RuntimeError(
            f"Epoch {epoch}: StudyInstanceUID " "is not one-to-one in OOF results."
        )
...
config = {
    ...
    "metadata_dim": SERIES_META_DIM,
    "slice_position_encoding": "normalized_dicom_physical_with_index_fallback",
    "slice_position_range": "[0,1]",
    "slice_position_hidden_dim": SLICE_POSITION_HIDDEN_DIM,
    "v3_controlled_change": "additive normalized slice position before slice attention",
    "labels": LABEL_COLUMNS,
}


with open(
    RESULT_ROOT / "v3_1_config.json",
    "w",
) as f:
