...
def decode_dicom_pixel_array(
    ...
    """
    Decode one MRI DICOM slice, with a narrow repair for a native
    uncompressed PixelData / NumberOfFrames metadata inconsistency.

    Normal path:
        ds.pixel_array

    Repair path:
        If PixelData length is an exact integer number of frames based
        on Rows * Columns * SamplesPerPixel * BitsAllocated / 8, but
        NumberOfFrames disagrees, temporarily correct NumberOfFrames
        in-memory and retry pydicom.

    We never pad/truncate pixel bytes, and we never guess for compressed
    transfer syntaxes. Ambiguous cases still raise.
    """
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                or samples <= 0
                or bits_allocated <= 0
                or bits_allocated % 8 != 0
            ):
                raise ValueError("Invalid native pixel metadata.")
            ...
            # PixelData may have one trailing pad byte.
            if (
                usable_bytes % bytes_per_frame != 0
                and usable_bytes > 0
            ...
            if (
                bytes_per_frame <= 0
                or usable_bytes <= 0
                or usable_bytes % bytes_per_frame != 0
            ):
                raise first_error
            ...
            if inferred_frames < 1 or inferred_frames == declared_frames:
                raise first_error
            ...
            print(
                ...
                f"{dicom_path} | "
                f"NumberOfFrames "
                f"{declared_frames} -> {inferred_frames} | "
                f"PixelData={actual_bytes} bytes"
            )

            if repaired.ndim != 2:
                raise RuntimeError(
                    "Repaired DICOM is not a single 2-D slice: "
                    f"{dicom_path}, shape={repaired.shape}"
                )
            ...
            raise RuntimeError(
                "DICOM pixel decode failed and safe metadata repair "
                "was not possible. "
                f"Path: {dicom_path}. "
