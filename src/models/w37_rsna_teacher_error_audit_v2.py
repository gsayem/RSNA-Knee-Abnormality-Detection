...
SCRIPT_VERSION = "rsna_teacher_error_audit_v2"
...
class Paths:
    ...
    def discover(cls) -> "Paths":
        ...
        output_root = results / "rsna_teacher_error_audit_v2"
...
def find_files_recursive(
    ...
    """
    Return ALL instances of filename.

    Fold-safe pipelines commonly store one copy under each:
        folds/fold_1/
        folds/fold_2/
        ...
    """
...
def restrict_prediction_to_gold_overlap(
    ...
    """
    Keep only gold UIDs that actually occur in this prediction file.

    A fold-specific held-out file is EXPECTED to contain only ~11-13
    of the 58 gold studies.
    """
...
def combine_fold_prediction_tables(
    pieces: Sequence[Tuple[Path, pd.DataFrame]],
    gold: pd.DataFrame,
    source_name: str,
) -> Tuple[pd.DataFrame, List[str]]:
    """
    Combine fold-held-out prediction tables into one 58-study OOF table.

    Duplicate UID predictions are allowed only when the actual
    probabilities are numerically identical; otherwise fail closed.
    """

    if not pieces:
        raise RuntimeError(f"{source_name}: no prediction pieces supplied.")
    ...
    if not frames:
        raise RuntimeError(f"{source_name}: no gold prediction rows found.")
    ...
    # -------------------------------------------------------------
    # Handle accidental duplicate copies safely.
    # -------------------------------------------------------------
    ...
    if combined.index.duplicated().any():
        ...
        for uid, group in combined.groupby(
            ...
            if not np.allclose(
                ...
                raise RuntimeError(
                    f"{source_name}: conflicting duplicate "
    ...
    if missing:
        raise RuntimeError(
            f"{source_name}: after combining fold artifacts, "
            f"still missing {len(missing)} gold UIDs. "
    ...
    if extras:
        combined = combined.loc[[uid for uid in combined.index if uid in expected]]

    combined = combined.loc[gold_order]

    if len(combined) != EXPECTED_GOLD:
        raise RuntimeError(
            f"{source_name}: expected {EXPECTED_GOLD} combined "
            f"OOF rows, got {len(combined)}"
        )
...
def load_w23_gold_predictions(
    ...
    if not sources:
        raise FileNotFoundError(
            "Could not locate any " "heldout_gold_stage_b_predictions.csv files"
        )

    log(f"W2.3 held-out files discovered: {len(sources)}")

    pieces: List[Tuple[Path, pd.DataFrame]] = []
    ...
    for source in sources:
        try:
            ...
            log(f"  {source.relative_to(root)} " f"-> {len(partial)} gold rows")
    ...
    # -------------------------------------------------------------
    # First check whether one artifact already contains all 58.
    # If not, concatenate fold-held-out files.
    # -------------------------------------------------------------

    full_candidates = [
        (path, frame) for path, frame in pieces if len(frame) == EXPECTED_GOLD
    ]

    if full_candidates:
        # Prefer a true aggregate artifact when available.
        source, wide = sorted(
            full_candidates,
...
def discover_fs4_gold_predictions(
    ...
    # -------------------------------------------------------------
    # First extract every CSV that contains usable FS4 predictions,
    # even when it contains only one held-out fold.
    # -------------------------------------------------------------
    ...
    for root in roots:
        for path in root.rglob("*.csv"):
            try:
                ...
                # Ignore production-only 4349-study tables that happen
                # to have no gold overlap.
                ...
                # fold_1/foo.csv ... fold_5/foo.csv belong together.
                # -------------------------------------------------
    ...
    # -------------------------------------------------------------
    # Candidate sets:
    #   A. standalone 58-row files
    #   B. fold files grouped by matching structural signature
    # -------------------------------------------------------------

    candidate_sets = []

    # A. Standalone complete artifact.
    ...
    for signature, items in groups.items():
        ...
        try:
            combined, used_sources = combine_fold_prediction_tables(
                pieces,
                gold,
                ("W2.6 FS4 candidate " f"{signature}"),
            )
    ...
    if not candidate_sets:
        raise RuntimeError(
            "FS4 files were found, but no standalone or "
            "combined candidate covered all 58 gold studies."
        )

    # -------------------------------------------------------------
    # Independently reconstruct replace and fixed50.
    # Known AUROC acts as a fail-closed identity guard.
    # -------------------------------------------------------------
    ...
    for candidate in candidate_sets:
        ...
        for p in paths_used:
            expected_values.append(candidate_expected_fixed_auc(Path(p)))

        # Prefer FAST benchmark only when the candidate actually
        # originates from FAST outputs.
        if any("fast" in str(p).lower() for p in paths_used):
    ...
    if best["distance"] > W26_TOLERANCE:
        ...
        raise RuntimeError(
            ...
            "No standalone/combined FS4 candidate "
            "reproduced the known fold-safe benchmark "
    ...
    meta.update(
        {
            "sources": [str(Path(p).resolve()) for p in best["paths"]],
