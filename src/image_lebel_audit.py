...
for _, repeated_group in (
    strict_repeats.iterrows()
):
    ...
    for col in STRICT_COLUMNS:
        ...
        target = normalize_signature_value(
            repeated_group[col]
        )
...
def compare_series(
    ...
    volume_a, datasets_a, positions_a = (
        read_series(
            study_uid,
            series_uid_a
        )
    ...
    volume_b, datasets_b, positions_b = (
        read_series(
            study_uid,
            series_uid_b
        )
    ...
    valid_b = [
        x
        for x in positions_b
        if x is not None
    ]


    result[
        "HasSpatialPositionsA"
    ] = bool(
        len(valid_a)
        == len(positions_a)
    )
    ...
    if (
        ...
        norm_a = robust_normalize(
            volume_a
        )

        norm_b = robust_normalize(
            volume_b
        )
    ...
    if (
        ...
        and
        result["PixelCorrelation"]
        is not None
        and
        result["PixelCorrelation"] > 0.995
        and
        result["PixelMAE"] < 0.01
    ):

        result[
            "Classification"
        ] = (
            "LIKELY_EXACT_OR_NEAR_EXACT_DUPLICATE"
        )
        ...
        is not None
        and
        result["PixelCorrelation"] > 0.90
    ):

        result[
            "Classification"
        ] = (
            "SAME_GEOMETRY_HIGHLY_SIMILAR_IMAGES"
        )
        ...
        result[
            ...
            "SAME_SHAPE_BUT_DIFFERENT_PIXELS"
        )
...
print(
    "IMAGE-LEVEL REPEATED-SERIES ANALYSIS"
)
...
comparison_df.to_csv(
    os.path.join(
        OUTPUT_DIR,
        "repeated_series_pixel_comparison.csv"
    ),
    index=False
)
...
representative_df.to_csv(
    os.path.join(
        OUTPUT_DIR,
        "dummy_blank_representative_series.csv"
    ),
    index=False
)
...
with open(
    os.path.join(
        OUTPUT_DIR,
        "image_level_audit_summary.json"
    ),
    "w"
) as f:
